...
export function getDikaRouteTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatDikaRouteCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-DikaRoute-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-DikaRoute-Provider` header already applies)
 ...
 */
export function buildDikaRouteDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildDikaRouteResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-DikaRoute-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-DikaRoute-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
}): Record<string, string> {
  const tokens = getDikaRouteTokenCounts(usage);
  const headers: Record<string, string> = {
    [DIKAROUTE_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [DIKAROUTE_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [DIKAROUTE_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatDikaRouteCost(costUsd)),
    [DIKAROUTE_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [DIKAROUTE_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [DIKAROUTE_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[DIKAROUTE_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[DIKAROUTE_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[DIKAROUTE_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[DIKAROUTE_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatDikaRouteCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[DIKAROUTE_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildDikaRouteDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[DIKAROUTE_RESPONSE_HEADERS.decision] = decisionValue;
  }

  return headers;
}

export function buildDikaRouteSseMetadataComment(
  options: Parameters<typeof buildDikaRouteResponseMetaHeaders>[0]
): string {
  const headers = buildDikaRouteResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-DikaRoute-* response meta headers.
 ...
 */
export function attachDikaRouteMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildDikaRouteResponseMetaHeaders>[0]
): void {
  const built = buildDikaRouteResponseMetaHeaders(meta);
...
/**
 * Attach the X-DikaRoute-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachDikaRouteMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildDikaRouteResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachDikaRouteMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachDikaRouteMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
