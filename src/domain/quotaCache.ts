...
import { getUsageForProvider } from "@dikaroute/open-sse/services/usage.ts";
...
import { getCodexQuotaWindowFilterForModel } from "@dikaroute/open-sse/config/codexQuotaScopes.ts";
import { getAntigravityQuotaFamily } from "@dikaroute/open-sse/services/antigravityQuotaFamily.ts";
...
declare global {
  var __dikarouteQuotaCacheState: QuotaCacheState | undefined;
}

function getState(): QuotaCacheState {
  if (!globalThis.__dikarouteQuotaCacheState) {
    globalThis.__dikarouteQuotaCacheState = {
      cache: new Map(),
  ...
  return globalThis.__dikarouteQuotaCacheState;
}
