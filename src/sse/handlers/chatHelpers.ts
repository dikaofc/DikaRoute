...
import { createBuiltinAutoCombo } from "@dikaroute/open-sse/services/autoCombo/builtinCatalog.ts";
import * as log from "../utils/logger";
import { updateProviderCredentials } from "../services/tokenRefresh";
import { detectFormatFromEndpoint } from "@dikaroute/open-sse/services/provider.ts";
import { resolveChatCoreTargetFormat } from "@dikaroute/open-sse/handlers/chatCore/targetFormat.ts";
import { handleChatCore } from "@dikaroute/open-sse/handlers/chatCore.ts";
import {
  checkResourcePressureGuard,
  type ResourcePressureGuardResult,
} from "@dikaroute/open-sse/utils/resourcePressure.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@dikaroute/open-sse/utils/error.ts";
import { HTTP_STATUS } from "@dikaroute/open-sse/config/constants.ts";
import {
  runWithProxyContext,
...
} from "@dikaroute/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@dikaroute/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@dikaroute/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-DikaRoute-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-DikaRoute-Session-Id", sessionId);
    return cloned;
  }
}
 ...
 * `x-dikaroute-modality-bridge` header on responses whose request payload was
...
export function withModalityBridgeHeader(response: Response, value: string | null): Response {
  ...
  try {
    response.headers.set("x-dikaroute-modality-bridge", value);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("x-dikaroute-modality-bridge", value);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-DikaRoute-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-DikaRoute-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
