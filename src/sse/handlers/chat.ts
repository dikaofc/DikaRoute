...
} from "@dikaroute/open-sse/services/accountFallback.ts";
import { getCombo, getComboForModel, getModelInfo } from "../services/model";
import { stripContextWindowSuffix } from "@dikaroute/open-sse/services/model.ts";
import { resolveBareModelToConnectionDefault } from "@dikaroute/open-sse/services/model.ts";
import { errorResponse } from "@dikaroute/open-sse/utils/error.ts";
import { getImageModelEntry } from "@dikaroute/open-sse/config/imageRegistry.ts";
import { acceptHeaderForcesStream } from "@dikaroute/open-sse/utils/aiSdkCompat.ts";
import { applyNoThinkingAlias } from "@dikaroute/open-sse/utils/noThinkingAlias.ts";
import { resolveCcDiscoveryAliasStrip } from "@/lib/ccDiscoveryAliasResolve";
import { handleComboChat, shouldSkipConnDisable } from "@dikaroute/open-sse/services/combo.ts";
import { mergeAbortSignals } from "@dikaroute/open-sse/executors/base.ts";
import { resolveRequestAutoControls } from "@dikaroute/open-sse/services/autoCombo/requestControls.ts";
import { resolveComboConfig } from "@dikaroute/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@dikaroute/open-sse/services/contextHandoff.ts";
import {
  HTTP_STATUS,
  ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE,
} from "@dikaroute/open-sse/config/constants.ts";
import { getTargetFormat, detectFormatFromUrl } from "@dikaroute/open-sse/services/provider.ts";
import {
  getModelsByProviderId,
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@dikaroute/open-sse/config/providerModels.ts";
...
import { connectionHasExtraKeys } from "@dikaroute/open-sse/services/apiKeyRotator.ts";
import {
...
import { isSubscriptionQuotaText } from "@dikaroute/open-sse/services/quotaTextCooldowns.ts";
...
import { cloneBoundedForLog } from "@dikaroute/open-sse/utils/requestLogger.ts";
import { handleInternalUsageCommand } from "@/lib/usage/internalUsageCommand";
import {
  applyTaskAwareRouting,
  getTaskRoutingConfig,
} from "@dikaroute/open-sse/services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "@dikaroute/open-sse/services/webSearchRouting.ts";
import {
...
import { startQuotaMonitor } from "@dikaroute/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@dikaroute/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@dikaroute/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@dikaroute/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@dikaroute/open-sse/services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "@dikaroute/open-sse/services/deepseekQuotaFetcher.ts";
import { registerOpenrouterQuotaFetcher } from "@dikaroute/open-sse/services/openrouterQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "@dikaroute/open-sse/services/opencodeQuotaFetcher.ts";
import { registerGrokWebQuotaFetcher } from "@dikaroute/open-sse/services/grokQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "@dikaroute/open-sse/services/genericQuotaFetcher.ts";
import "@dikaroute/open-sse/services/quotaTrackersBatch.ts";
import {
...
async function handleChatImplementation(
  ...
  // DikaRoute-level 400 before any routing or upstream call (#5110, #6402).
  ...
  const requestedConnectionId = request.headers.get("x-dikaroute-connection")?.trim() || null;
  if (sessionId) {
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._dikarouteSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._dikarouteInternalRequest !== "context-handoff") {
