...
import { getQuotaScopeLabelForProvider } from "@dikaroute/open-sse/services/antigravityQuotaFamily.ts";
import { getCreditsMode } from "@dikaroute/open-sse/services/antigravityCredits.ts";
import {
...
import { isLocalProvider } from "@dikaroute/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@dikaroute/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@dikaroute/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@dikaroute/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@dikaroute/open-sse/services/errorClassifier.ts";
...
} from "@dikaroute/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-dikaroute-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / DikaRoute / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against DikaRoute keys.
 *
 ...
 * Feature #1350: Supports DIKAROUTE_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.DIKAROUTE_API_KEY || process.env.ROUTER_API_KEY;
