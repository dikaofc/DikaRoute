...
/**
 * Rename a Node process title so DikaRoute is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `dikaroute${currentTitle.slice("next-server".length)}`;
}
...
function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.DIKAROUTE_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  ...
  // Rename the process title so DikaRoute is identifiable in ps/htop instead
  ...
  await import("@dikaroute/open-sse/index.ts");
  ...
  // See: https://github.com/dikaofc/DikaRoute/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@dikaroute/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@dikaroute/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } =
      await import("@dikaroute/open-sse/services/thinkingBudget.ts");
    ...
    const { hydrateTaskRoutingConfig } =
      await import("@dikaroute/open-sse/services/taskAwareRouter.ts");
  ...
  // DIKAROUTE_DISABLE_CREDENTIAL_HEALTH_CHECK and its cadence is tunable via
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    await Promise.allSettled([
      ...
      import("@dikaroute/open-sse/services/autoRefreshDaemon")
      ...
      // Reads the schedule written by `dikaroute backup auto enable` and fires
      ...
      // flag-gated auto-start (DIKAROUTE_ENABLE_LIVE_WS, default ON).
