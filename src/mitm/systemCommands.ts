 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by DikaRoute's runtime
...
/**
 * Truthy-env check for `DIKAROUTE_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.DIKAROUTE_NO_SUDO;
 ...
 * the operator opts into root-less mode via `DIKAROUTE_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `DIKAROUTE_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dikaroute-elevate-"));
  const scriptName = `dikaroute-elevate-${crypto.randomUUID()}.ps1`;
