...
const ROUTER_BASE_URL = (
  process.env.DIKAROUTE_BASE_URL ||
...
// Inspector. The token is injected by manager.ts (same value the DikaRoute
 ...
 * shape. The route-only namespace is reserved for client-facing DikaRoute model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-dikaroute-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The DikaRoute router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-dikaroute-source": "agent-bridge",
        "x-dikaroute-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`DikaRoute ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-dikaroute-source"] === "dikaroute") {
      vlog(1, `[MITM] → PASSTHROUGH (DikaRoute source loop)`);
