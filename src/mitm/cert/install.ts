...
const LINUX_CERT_NAME = "dikaroute-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "DikaRoute MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.DIKAROUTE_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] DIKAROUTE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `dikaroute-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `dikaroute-tproxy-ca.crt` slot
...
// They install / remove the DikaRoute MITM root CA from the OS trust store and
