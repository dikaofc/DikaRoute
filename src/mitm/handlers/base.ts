 ...
 *   - forwarding to the DikaRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@dikaroute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@dikaroute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the DikaRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the DikaRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-dikaroute-source`, `x-dikaroute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.DIKAROUTE_BASE_URL ?? "http://127.0.0.1:20128";
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-dikaroute-agent": this.agentId,
