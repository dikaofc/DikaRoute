 ...
 * `thinkingConfig`, …). The DikaRoute router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the DikaRoute router,
 ...
 * @param model      resolved DikaRoute model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`DikaRoute ${upstream.status}: ${errText}`);
      }
