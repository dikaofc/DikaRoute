 ...
 * trust store under a DEDICATED slot (`dikaroute-tproxy-ca.crt`), separate from
 * the static MITM cert (`dikaroute-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "dikaroute-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.DIKAROUTE_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] DIKAROUTE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
