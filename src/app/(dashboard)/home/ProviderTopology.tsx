...
function RouterNode({ data }: { data: RouterNodeData }) {
  return (
    <div className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-primary bg-primary/8 shadow-lg min-w-[140px] justify-center">
      ...
      <span className="text-sm font-bold text-primary">DikaRoute</span>
      {data.activeCount > 0 && (
...
export default function ProviderTopology({
  ...
  if (providers.length === 0) {
    return (
      <div
        className={`${containerClass} relative flex flex-col items-center justify-center gap-5 p-6 text-center sm:p-8`}
      >
        <div className="dkr-hero-grid pointer-events-none absolute inset-0 opacity-50" />
        <div className="pointer-events-none absolute -top-20 left-1/2 size-60 -translate-x-1/2 rounded-full bg-primary/10 blur-3xl" />

        <span className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-primary/15 text-primary shadow-[0_0_0_4px_rgba(10,132,255,0.08)]">
          <span className="material-symbols-outlined text-[24px]">device_hub</span>
        </span>

        <div className="relative flex flex-col gap-1.5">
          <p className="text-sm font-semibold text-text-main">No providers connected yet</p>
        ...
        <div className="relative flex flex-col items-center gap-2 sm:flex-row sm:gap-3">
          <div className="flex items-center gap-2 rounded-lg border border-dashed border-border bg-bg-subtle/40 px-3 py-2">
            <span className="material-symbols-outlined text-[14px] text-primary">route</span>
            <span className="font-mono text-[11px] text-text-main">Gateway</span>
          </div>
          <span className="material-symbols-outlined text-[14px] text-text-muted sm:rotate-0 rotate-90">
            arrow_forward
          </span>
          <div className="flex items-center gap-2 rounded-lg border border-dashed border-primary/25 bg-primary/[0.04] px-3 py-2">
            <span className="dkr-status-dot size-1.5 rounded-full bg-primary" />
            <span className="font-mono text-[11px] text-text-main">Provider</span>
          </div>
          <span className="material-symbols-outlined text-[14px] text-text-muted sm:rotate-0 rotate-90">
            arrow_forward
          </span>
          <div className="flex items-center gap-2 rounded-lg border border-dashed border-border bg-bg-subtle/40 px-3 py-2">
            <span className="material-symbols-outlined text-[14px] text-text-muted">model_training</span>
            <span className="font-mono text-[11px] text-text-muted">Models</span>
          </div>
        </div>

        <Link href="/dashboard/providers" prefetch={false}>
