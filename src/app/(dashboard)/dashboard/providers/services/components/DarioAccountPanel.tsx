 ...
 * DARIO_ADMIN_TOKEN never reaches this component; the DikaRoute routes attach it.
 *
 ...
 *
 * Also offers "Import from DikaRoute": lists any existing DikaRoute `claude`
 ...
 * /api/services/dario/admin/import-from-dikaroute/route.ts for why this is
...
export function DarioAccountPanel() {
  ...
  const refreshOmniConnections = useCallback(async () => {
    setOmniLoading(true);
    try {
      const res = await fetch("/api/services/dario/admin/import-from-dikaroute");
      const json = (await res.json().catch(() => null)) as {
  ...
  async function importFromDikaroute(connectionId: string) {
  ...
  return (
    <Card padding="md">
      <div className="space-y-4">
        ...
        {/* Import from DikaRoute */}
        <div className="space-y-2 border-t border-border pt-3">
          <p className="text-xs font-medium">Import from DikaRoute</p>
          <p className="text-xs text-text-muted">
            Reuse an existing DikaRoute Claude connection&apos;s OAuth tokens instead of logging in
          ...
          {omniLoading && omniConnections.length === 0 ? (
            ...
            <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2">
              <p className="text-xs text-text-muted">
                No eligible DikaRoute Claude connections found.
              </p>
              <Tooltip content="Will be active once a Claude connection exists in DikaRoute.">
            ...
            omniConnections.map((c) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => void importFromDikaroute(c.id)}
                >
