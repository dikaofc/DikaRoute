...
import isCheaperInferenceProviderId from "../featuredProviders";
...
const ProviderCard = forwardRef<ProviderCardHandle, ProviderCardProps>(function ProviderCard(
  ...
  const isSponsorPartner = isCheaperInferencePartner;
  ...
  const cheaperInferenceSupporterChip = isCheaperInferencePartner ? (
    <span
      ...
      title={providerText(
        ...
        "Cheaper Inference backs DikaRoute as an Open Source Friend"
      )}
    >
  ...
  const staticIconPath = (() => {
    if (isCompatible) {
      return provider.apiType === "responses" ? "/providers/oai-r.svg" : "/providers/oai-cc.svg";
    }
    if (isAnthropicCompatible || isCcCompatible) return "/providers/anthropic-m.svg";
    return null;
  })();
