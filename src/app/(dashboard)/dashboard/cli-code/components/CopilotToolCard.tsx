 ...
 *
 * Feature request: https://github.com/dikaofc/DikaRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("dikaroute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("dikaroute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "dikaroute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "DikaRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.dikaroute}`,
      models,
    };

    const responsesConfig = {
      name: "DikaRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.dikaroute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("dikaroute-cli-key-copilot", value);
  };
