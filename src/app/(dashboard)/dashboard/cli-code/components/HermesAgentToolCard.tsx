...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "DikaRoute") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "DikaRoute" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as DikaRoute
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingDikaRoute: true };
  ...
  // Count of roles that are (or will be) routed via DikaRoute
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as DikaRoute intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingDikaRoute) and settings shape (provider + base_url)
    if (typeof info.usingDikaRoute === "boolean") {
      return info.usingDikaRoute;
    }
    return (
      info?.provider === "dikaroute" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via DikaRoute
              // - Else if current from disk: show real provider name + "(not DikaRoute)" or "DikaRoute"
              let badge: { label: string; pending: boolean; outsideDikaRoute: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via DikaRoute
                const prov = sel.provider || "DikaRoute";
                badge = {
                  label: t("hermesViaDikaRoute", { provider: prov }),
                  pending: true,
                  outsideDikaRoute: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "dikaroute" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "DikaRoute", pending: false, outsideDikaRoute: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotDikaRoute", { provider: realProvider }),
                    pending: false,
                    outsideDikaRoute: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "DikaRoute";
