...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:DikaRoute-<i> entry (multi-model).
  const isDikaRouteEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:DikaRoute");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isDikaRouteEntry);
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/droid-settings", {
        ...
        body: JSON.stringify({
          ...
          apiKey: !cloudEnabled ? "sk_dikaroute" : null,
          keyId: selectedKeyId,
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_dikaroute" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first DikaRoute entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isDikaRouteEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? t("noApiKeysCreateOne") : t("defaultDikarouteKey")}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasDikaRoute}
                  loading={restoring}
                >
