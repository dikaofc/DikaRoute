...
import { getAllEmbeddingModels } from "@dikaroute/open-sse/config/embeddingRegistry";
import {
  getAllImageModels,
  isRegisteredImageModel,
} from "@dikaroute/open-sse/config/imageRegistry";
import { getAllRerankModels } from "@dikaroute/open-sse/config/rerankRegistry";
import { getAllAudioModels } from "@dikaroute/open-sse/config/audioRegistry";
import { getAllModerationModels } from "@dikaroute/open-sse/config/moderationRegistry";
import { getAllVideoModels } from "@dikaroute/open-sse/config/videoRegistry";
import { getAllMusicModels } from "@dikaroute/open-sse/config/musicRegistry";
import { REGISTRY } from "@dikaroute/open-sse/config/providerRegistry";
import { CODEX_NATIVE_UNPREFIXED_MODELS } from "@dikaroute/open-sse/services/model";
import { resolveNestedComboTargets } from "@dikaroute/open-sse/services/combo";
import {
  ...
  isPaidTierAutoId,
} from "@dikaroute/open-sse/services/autoCombo/builtinCatalog";
...
import { getTokenLimit } from "@dikaroute/open-sse/services/contextManager";
...
import { buildErrorBody } from "@dikaroute/open-sse/utils/error";
  ...
  try {
    ...
    if (apiKey) {
      ...
      if (keyMeta && keyMeta.allowedQuotas && keyMeta.allowedQuotas.length > 0) {
        ...
        // (DIKAROUTE_API_KEY / ROUTER_API_KEY per isValidApiKey). Those keys have no
