...
import { parseModel, resolveCanonicalProviderModel } from "@dikaroute/open-sse/services/model";
...
/** True when `${prefix}/__dikaroute_probe__` parses back to the given providerId. */
export function prefixRoutesToProvider(prefix: string, providerId: string): boolean {
  const parsed = parseModel(`${prefix}/__dikaroute_probe__`);
