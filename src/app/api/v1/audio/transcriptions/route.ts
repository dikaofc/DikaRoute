...
import { handleAudioTranscription } from "@dikaroute/open-sse/handlers/audioTranscription.ts";
import {
...
import { attachDikaRouteMetaToResponse } from "@/domain/dikarouteResponseMeta";
...
import { log } from "@dikaroute/open-sse/utils/logger.ts";
...
async function transcribeWithModel(
  ...
  if (response?.ok) {
    ...
    response = attachDikaRouteMetaToResponse(response, {
      provider,
      model: resolvedModel,
      costUsd: 0,
