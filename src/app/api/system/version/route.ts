...
import { resolveGlobalDikaroutePath } from "@/lib/system/globalPackagePath";
...
function getCurrentVersion(): string {
  // Runtime read: the standalone server lives in <pkg>/dist while the CLI
  // root package.json (the version users actually installed) sits one level
  // up. A build-time require() bakes a stale version into the compiled
  // bundle, so after `dikaroute update` the dashboard keeps nagging
  // "Update Available" for the version that is already installed.
  try {
    const candidates = [
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          try {
            await execFileAsync("pm2", ["restart", "dikaroute", "--update-env"], {
              timeout: 30_000,
              cwd: PROJECT_ROOT,
            });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        send({ step: "install", status: "running", message: `Installing dikaroute@${latest}...` });
          await execFileAsync(
            "npm",
            ["install", "-g", `dikaroute@${latest}`, "--ignore-scripts", "--legacy-peer-deps"],
            buildNpmExecOptions(process.platform, { cwd: PROJECT_ROOT, timeoutMs: 300_000 })
          );
        send({ step: "install", status: "done", message: `Installed dikaroute@${latest}` });
        ...
        const dikaPath = await resolveGlobalDikaroutePath();
        await execFileAsync(
          "npm",
          ["rebuild", "better-sqlite3"],
          buildNpmExecOptions(process.platform, { cwd: dikaPath, timeoutMs: 120_000 })
        );
