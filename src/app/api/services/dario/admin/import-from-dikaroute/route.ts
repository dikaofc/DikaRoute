/**
 * GET/POST /api/services/dario/admin/import-from-dikaroute
 *
 * Imports an existing DikaRoute `claude` provider connection's OAuth tokens
 * directly into Dario's account store, skipping the interactive browser OAuth
 * flow entirely. This works because DikaRoute's native `claude` provider and
 ...
 * PKCE/token-exchange ourselves, since DikaRoute's own decrypt() already
 ...
 * the file we stop+start the DikaRoute-managed supervisor to force a clean
...
function safeAliasFromSource(email: string | null | undefined, connectionId: string): string {
  const base = (email || connectionId || "dikaroute").toLowerCase();
  const cleaned = base.replace(/[^a-z0-9_.-]/g, "-").replace(/^[^a-z0-9]+/, "");
  const alias = cleaned || "dikaroute";
  return `dikaroute-${alias}`.slice(0, 64);
}
