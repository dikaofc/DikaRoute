 ...
 *
 * Re-issues the captured request through the local DikaRoute instance and
...
const DIKAROUTE_BASE = process.env.DIKAROUTE_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${DIKAROUTE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-dikaroute-source": "inspector-replay",
  };
