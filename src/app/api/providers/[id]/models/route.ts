...
import { getRegistryEntry } from "@dikaroute/open-sse/config/providerRegistry.ts";
...
import { getStaticQoderModels } from "@dikaroute/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import { resolveZedModels } from "@dikaroute/open-sse/shared/zedAuth.ts";
import {
  fetchGitHubCopilotModels,
  fetchGheCopilotModels,
} from "@dikaroute/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@dikaroute/open-sse/services/kiroModels.ts";
import {
  buildGlmCodingHeaders,
  buildGlmModelsUrl,
} from "@dikaroute/open-sse/config/glmProvider.ts";
import { getImageProvider } from "@dikaroute/open-sse/config/imageRegistry.ts";
import { getVideoProvider } from "@dikaroute/open-sse/config/videoRegistry.ts";
import {
  discoverBedrockNativeModels,
  isBedrockNativeApiError,
} from "@dikaroute/open-sse/services/bedrock.ts";
import {
  discoverPromptQlModels,
  PROMPTQL_FALLBACK_MODELS,
} from "@dikaroute/open-sse/services/promptqlModels.ts";
import {
  discoverNotionWebModels,
  NOTION_WEB_FALLBACK_MODELS,
} from "@dikaroute/open-sse/services/notionWebModels.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@dikaroute/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@dikaroute/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@dikaroute/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@dikaroute/open-sse/config/sap.ts";
import {
  WATSONX_DEFAULT_BASE_URL,
  buildWatsonxModelsUrl,
} from "@dikaroute/open-sse/config/watsonx.ts";
import { getEmbeddingProvider } from "@dikaroute/open-sse/config/embeddingRegistry.ts";
import { getRerankProvider } from "@dikaroute/open-sse/config/rerankRegistry.ts";
import {
  getSpeechProvider,
  getTranscriptionProvider,
} from "@dikaroute/open-sse/config/audioRegistry.ts";
import {
...
import { fetchRaycastModels } from "@dikaroute/open-sse/services/raycast.ts";
import { runWithProxyContext } from "@dikaroute/open-sse/utils/proxyFetch.ts";
import {
  type JsonRecord,
  asRecord,
  toNonEmptyString,
...
export async function GET(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      // the local/cached catalog DikaRoute ships instead of surfacing a raw 503.
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@dikaroute/open-sse/executors/vertex.ts");
        if (accessToken) {
