...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `dikaroute-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `dikaroute-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "dikaroute-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="dikaroute-full-backup-${timestamp}.tar.gz"`,
