...
/**
 * Render the DikaRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by DikaRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-DikaRoute marker comment.
 */
...
// POST — write DikaRoute settings to Forge config.toml
...
// DELETE — remove Forge DikaRoute config
