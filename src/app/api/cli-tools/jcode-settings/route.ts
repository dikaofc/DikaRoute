...
/**
 * Check if the config file contains DikaRoute settings.
 */
...
// POST — write DikaRoute settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DikaRoute settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove DikaRoute settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DikaRoute-managed fields
    ...
    return NextResponse.json({ success: true, message: "jcode DikaRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
