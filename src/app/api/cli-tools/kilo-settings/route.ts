...
// Check if DikaRoute OpenAI-compatible provider is configured
const hasDikaRouteConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["dikaroute"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("dikaroute")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDikaRoute: hasDikaRouteConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use DikaRoute as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update DikaRoute as openai-compatible provider
...
// DELETE - Remove DikaRoute config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DikaRoute provider
    delete auth["openai-compatible"];
    delete auth["dikaroute"];
    ...
    return NextResponse.json({
      success: true,
      message: "DikaRoute settings removed from Kilo Code",
    });
  } catch (error) {
