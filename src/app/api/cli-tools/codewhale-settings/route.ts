...
/**
 * Render the DikaRoute config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by DikaRoute (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasDikaRouteConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by DikaRoute");
};
...
// POST — write DikaRoute settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove DikaRoute CodeWhale config (primary + legacy, if present)
