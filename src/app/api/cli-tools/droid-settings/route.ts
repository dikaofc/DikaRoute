...
// Check if settings has DikaRoute customModels.
// Multi-model entries are stored as `custom:DikaRoute-0`, `custom:DikaRoute-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasDikaRouteConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isDikaRouteCustomModel);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasDikaRoute: hasDikaRouteConfig(settings),
...
// POST - Update DikaRoute customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing DikaRoute config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isDikaRouteCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend DikaRoute entries (one per requested model)
...
// DELETE - Remove DikaRoute customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DikaRoute customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isDikaRouteCustomModel(m));
