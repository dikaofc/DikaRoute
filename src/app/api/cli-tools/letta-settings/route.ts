...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.dikaroute-backup");
...
// ── Check if a base_url points to DikaRoute ──────────────────────────────
const isDikaRouteUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("dikaroute");
};

// ── Check if DikaRoute is configured ─────────────────────────────────────
const hasDikaRouteConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isDikaRouteUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-DikaRoute endpoint
    let lmstudioConflict = false;
    if (provider && !isDikaRouteUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasDikaRoute: hasDikaRouteConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply DikaRoute as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-DikaRoute provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isDikaRouteUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isDikaRouteUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-DikaRoute conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-dikaroute provider if present
    if (authFile.providers?.["lc-dikaroute"]) {
      delete authFile.providers["lc-dikaroute"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a DikaRoute model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove DikaRoute configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "DikaRoute config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "DikaRoute config removed. Restart Letta CLI to take effect.";
