  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the DikaRoute settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the DikaRoute model entry
  ...
  // Check if DikaRoute entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.dikarouteManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("dikaroute") ||
 ...
 * keys, but make sure the DikaRoute provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      dikaroute: {
        ...((existingConfig.providers && existingConfig.providers.dikaroute) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.dikaroute &&
            existingConfig.providers.dikaroute.api_key) ||
          "",
      },
    },
  };
