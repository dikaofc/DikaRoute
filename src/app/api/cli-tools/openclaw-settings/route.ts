...
// Check if settings has DikaRoute config
const hasDikaRouteConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["dikaroute"];
};
...
// POST - Update DikaRoute settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `dikaroute/${model}`;

    // Update models.providers.dikaroute
    settings.models.providers["dikaroute"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DikaRoute from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["dikaroute"];
    ...
    // Reset agents.defaults.model.primary if it uses dikaroute
    if (settings.agents?.defaults?.model?.primary?.startsWith("dikaroute/")) {
