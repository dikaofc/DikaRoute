...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.dikaroute]
...
// Check if config has DikaRoute settings
const hasDikaRouteConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "dikaroute"') ||
    config.includes("[model_providers.dikaroute]")
  );
};
...
// POST - Update DikaRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only DikaRoute related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use DIKAROUTE_API_KEY
    parsed._root.model_provider = "dikaroute";
    parsed._sections["model_providers.dikaroute"] = {
      name: "DikaRoute",
...
// DELETE - Remove DikaRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DikaRoute related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "dikaroute") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove dikaroute provider section
    delete parsed._sections["model_providers.dikaroute"];
    ...
    return NextResponse.json({
      success: true,
      message: "DikaRoute settings removed successfully",
    });
  } catch (error) {
