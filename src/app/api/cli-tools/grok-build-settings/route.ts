...
// [model.dikaroute] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# dikaroute-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields DikaRoute itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "DikaRoute"`,
    `description = "Routed via DikaRoute gateway"`,
...
/** Insert/replace the `[model.dikaroute]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# dikaroute-prev-default = "${current}"\n`;
...
const hasDikaRouteConfig = (modelCfg: GrokModelSection | null): boolean =>
...
// GET — check Grok Build CLI and return current [model.dikaroute] config
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDikaRoute: hasDikaRouteConfig(model),
...
// POST — write the [model.dikaroute] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.dikaroute] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "DikaRoute model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
