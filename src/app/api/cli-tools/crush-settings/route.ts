...
import { sanitizeErrorMessage } from "@dikaroute/open-sse/utils/error.ts";
...
// the `dikaroute setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. DikaRoute is registered under
 * the `dikaroute` provider id as an `openai-compat` provider — same shape
...
const hasDikaRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const dikaroute = providers?.dikaroute as Record<string, unknown> | undefined;
  return (
    !!dikaroute &&
    dikaroute.type === "openai-compat" &&
    typeof dikaroute.base_url === "string" &&
    dikaroute.base_url.length > 0
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasDikaRoute: hasDikaRouteConfig(config),
...
// POST — write DikaRoute settings to crush.json (providers.dikaroute)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `DikaRoute: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove DikaRoute provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush DikaRoute settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
