...
import { estimateTokens } from "@dikaroute/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@dikaroute/open-sse/services/compression/bodyAdapter.ts";
import type {
  CompressionConfig,
  CompressionResult,
} from "@dikaroute/open-sse/services/compression/types.ts";
import { resolveCompressionSettings } from "@dikaroute/open-sse/handlers/chatCore/compressionSettings.ts";
import {
  writeCompressionAnalytics,
  writeCompressionSkip,
} from "@dikaroute/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } = await import(
      "@dikaroute/open-sse/services/compression/strategySelector.ts"
    );
