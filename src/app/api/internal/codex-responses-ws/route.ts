...
import { z } from "zod";
import { CodexExecutor } from "@dikaroute/open-sse/executors/codex.ts";
...
import { logger } from "@dikaroute/open-sse/utils/logger.ts";
import { resolveProxy } from "@dikaroute/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@dikaroute/open-sse/utils/proxyDispatcher.ts";
import {
...
function getBridgeSecret(): string {
  return process.env.DIKAROUTE_WS_BRIDGE_SECRET || "";
}
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://dikaroute.local");
...
async function resolveCodexRequestContext(body: JsonRecord) {
  if (!isFeatureFlagEnabled("DIKAROUTE_CODEX_WS_ENABLED")) {
    return {
...
async function prepare(body: JsonRecord) {
  ...
  if (reasoningDecision) {
    ...
    reasoningRouting = isRecord(withDirective._dikarouteReasoningRouteTrace)
      ? withDirective._dikarouteReasoningRouteTrace
      : null;
    responseBodyWithMemory = applyReasoningRuleDirective(withDirective) as JsonRecord;
    delete responseBodyWithMemory._dikarouteReasoningRouteTrace;
  }
  ...
  // Responses WebSocket too. The downstream client→DikaRoute hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-dikaroute-ws-bridge-secret") || "";
