...
import { getClaudeCodeDefaultModels } from "@dikaroute/open-sse/config/providerRegistry";
...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  claude: {
    ...
    color: "#D97757",
    description: "Anthropic Claude Code CLI — ANTHROPIC_BASE_URL points to DikaRoute",
  ...
  codex: {
    ...
    color: "#10A37F",
    description: "OpenAI Codex CLI — OpenAI-compatible base URL targets DikaRoute",
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model dikaroute/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  hermes: {
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Use the JSON block below as the OpenAI-compatible provider definition for DikaRoute.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  qwen: {
    ...
    color: "#30D158",
    description: "Qwen Code CLI — current V4 OpenAI-compatible model provider via DikaRoute",
    ...
    notes: [
      {
        type: "info",
        text: "DikaRoute is registered under modelProviders.openai using Qwen Code's current bare-array V4 format.",
      },
      {
        type: "info",
        text: "The API key is stored only as DIKAROUTE_API_KEY in ~/.qwen/.env, leaving your existing provider credentials untouched.",
      },
    ],
    guideSteps: [
  ...
  "modelProviders": {
    "openai": [
      {
        "id": "{{model}}",
        "name": "{{model}} (DikaRoute)",
        "envKey": "DIKAROUTE_API_KEY",
        "baseUrl": "{{baseUrl}}"
      }
    ]
  },
...
OPENAI_API_KEY: "{{apiKey}}"`,
  ...
  omp: {
    id: "omp",
    name: "Oh My Pi",
    image: "/providers/omp.svg",
    color: "#111111",
    docsUrl: "https://github.com/can1357/oh-my-pi",
    description: "Oh My Pi terminal coding agent via DikaRoute",
    configType: "custom",
    category: "agent",
    vendor: "OSS",
    acpSpawnable: true,
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi reads custom OpenAI-compatible providers from ~/.omp/agent/models.yml. DikaRoute adds itself as a provider with auto-discovery — models appear automatically in omp's /model menu.",
      },
      {
        type: "warning",
  ...
  letta: {
    ...
    notes: [
      {
        type: "info",
        text: "Letta CLI uses pi-ai which sends OpenAI-compatible requests. DikaRoute configures it as an OpenAI provider with custom base URL.",
      },
      {
        type: "info",
        text: "CLI (Local Mode): DikaRoute auto-configures ~/.letta/lc-local-backend/providers/auth.json. Use 'letta --info' to check if local mode is enabled.",
      },
      {
        type: "warning",
