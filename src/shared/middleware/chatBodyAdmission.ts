...
export const CHAT_LARGE_BODY_BYTES = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_LARGE_BODY_BYTES,
  256 * 1024
);

export const CHAT_HARD_MAX_BODY_BYTES = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_HARD_MAX_BODY_BYTES,
  50 * 1024 * 1024
);

const CHAT_MAX_HEAVY_IN_FLIGHT = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_MAX_HEAVY_IN_FLIGHT,
  1
);

export const CHAT_HEAVY_MESSAGE_COUNT = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_HEAVY_MESSAGE_COUNT,
  200
);
export const CHAT_HEAVY_TOOL_COUNT = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_HEAVY_TOOL_COUNT,
  64
);
export const CHAT_HEAVY_ESTIMATED_TOKENS = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_HEAVY_ESTIMATED_TOKENS,
  32_000
);
/**
 ...
 * Enforcing one here rejected conversations before DikaRoute's own compression pipeline — the
 ...
 * entries, so the metric an operator caps is partly manufactured by DikaRoute itself.
 *
 ...
 * ceiling opt in with `DIKAROUTE_CHAT_HARD_MAX_MESSAGES`.
 */
export const CHAT_HARD_MAX_MESSAGES = parsePositiveInt(
  process.env.DIKAROUTE_CHAT_HARD_MAX_MESSAGES,
  0
);
 ...
 * synchronous JavaScript turn, making acquisition atomic within an DikaRoute process.
 ...
 * local-mode `sk_dikaroute` sentinel or the operator-configured env key
 * (`DIKAROUTE_API_KEY` / `ROUTER_API_KEY`, #1350) so REQUIRE_API_KEY=true
 * deployments can run the describe sub-request.
 */
export const ADMISSION_BYPASS_HEADER = "x-dikaroute-admission-bypass";
const ADMISSION_BYPASS_VALUE = "internal";
const SELF_LOOP_KEY = "sk_dikaroute";

/**
 * Resolve the bearer credential used by trusted in-process self-loop
 * sub-requests (the vision-bridge describe call).
 *
 * Local mode uses the `sk_dikaroute` sentinel. Deployments that force API key
 * auth (`REQUIRE_API_KEY=true`) reject that sentinel with 401, so they must use
 * a real key — the persistent env-var key (#1350, `DIKAROUTE_API_KEY` /
...
export function resolveSelfLoopBearer(): string {
  return (
    process.env.DIKAROUTE_API_KEY?.trim() || process.env.ROUTER_API_KEY?.trim() || SELF_LOOP_KEY
  );
}
...
function isInternalAdmissionBypass(request: Request): boolean {
  ...
  // the local `sk_dikaroute` sentinel OR the operator-configured env key
  // (`DIKAROUTE_API_KEY` / `ROUTER_API_KEY`, #1350) so REQUIRE_API_KEY=true
