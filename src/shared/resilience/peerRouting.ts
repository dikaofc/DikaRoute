...
export const DIKAROUTE_PEER_TRACE_HEADER = "X-DikaRoute-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  DIKAROUTE_INSTANCE_ID?: string;
  DIKAROUTE_PEER_URLS?: string;
  DIKAROUTE_PEER_MAX_HOPS?: string;
};
...
function getInstanceId(env: PeerEnvironment): string | null {
  const value = env.DIKAROUTE_INSTANCE_ID?.trim() ?? "";
  return INSTANCE_ID_PATTERN.test(value) ? value : null;
}

function getMaxPeerHops(env: PeerEnvironment): number {
  const parsed = Number.parseInt(env.DIKAROUTE_PEER_MAX_HOPS ?? "", 10);
...
export function isConfiguredDikaRoutePeer(
  targetUrl: string,
  ...
  return (env.DIKAROUTE_PEER_URLS ?? "")
    .split(",")
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, DIKAROUTE_PEER_TRACE_HEADER));
  if (trace.includes(instanceId)) {
    return {
      code: "peer_loop_detected",
      message: "DikaRoute peer routing loop detected",
    };
  }
  if (trace.length >= getMaxPeerHops(env)) {
    return {
      code: "peer_hop_limit_exceeded",
      message: "DikaRoute peer routing hop limit exceeded",
    };
  }
  return null;
}
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted DikaRoute URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredDikaRoutePeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, DIKAROUTE_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = DIKAROUTE_PEER_TRACE_HEADER.toLowerCase();
