...
export const QWEN_CODE_ENV_KEY = "DIKAROUTE_API_KEY";
...
/**
 * Identifies only entries owned by DikaRoute. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "dikaroute") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (DikaRoute)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (DikaRoute)`,
...
/**
 * Merge one DikaRoute model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "dikaroute") delete next.selectedProvider;

  return next;
};

export const findDikaRouteQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasDikaRouteQwenCodeConfig = (settings: unknown): boolean =>
  findDikaRouteQwenCodeModel(settings) !== undefined;

/** Remove only DikaRoute-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?DIKAROUTE_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_dikaroute"))}`);
  return `${lines.join("\n")}\n`;
};
