...
export function resolveModelSyncInternalBaseUrl(_candidate?: string): string {
  ...
  const nativeTls = process.env.DIKAROUTE_INTERNAL_SCHEME === "https";
  const origin = nativeTls
    ? `https://localhost:${dashboardPort}`
    : `http://127.0.0.1:${dashboardPort}`;
  return `${origin}${normalizeInternalBasePath(process.env.DIKAROUTE_BASE_PATH)}`;
}
...
const globalState = globalThis as typeof globalThis & {
  __dikarouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__dikarouteModelSyncInternalAuthToken || randomUUID();
    globalState.__dikarouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__dikarouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__dikarouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call DikaRoute's own API
