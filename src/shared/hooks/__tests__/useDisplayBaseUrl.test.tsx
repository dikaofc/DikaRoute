...
describe("useDisplayBaseUrl", () => {
  ...
  it("appends DIKAROUTE basePath when resolving a public browser origin", () => {
    expect(
      resolveDisplayBaseUrl(
    ...
    ).toBe("https://api.example.com/dikaroute");
  });

  it("keeps a configured public URL that already includes the basePath", () => {
    expect(
      resolveDisplayBaseUrl(
        "https://api.example.com/dikaroute",
        "https://api.example.com",
        "/dikaroute/dashboard",
        "/dikaroute"
      )
  ...
  it("derives basePath from NEXT_PUBLIC_BASE_URL when env basePath is unset", () => {
    expect(
      resolveDisplayBaseUrl("https://api.example.com/dikaroute", "https://api.example.com", "/")
    ).toBe("https://api.example.com/dikaroute");
  });

  it("normalizes basePath values without a leading slash", () => {
    expect(normalizeBasePath("dikaroute")).toBe("/dikaroute");
    expect(normalizeBasePath("/dikaroute/")).toBe("/dikaroute");
