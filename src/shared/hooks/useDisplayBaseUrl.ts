...
/**
 * Resolve the deploy basePath for display URLs (e.g. `/dikaroute`).
 *
 * Priority:
 * 1. Explicit `envBasePath` argument / `NEXT_PUBLIC_DIKAROUTE_BASE_PATH`
  ...
  const fromEnv = normalizeBasePath(
    envBasePath ??
      (typeof process !== "undefined"
        ? process.env.NEXT_PUBLIC_DIKAROUTE_BASE_PATH || process.env.DIKAROUTE_BASE_PATH
        : undefined)
  );
 ...
 * @param envBasePath       `NEXT_PUBLIC_DIKAROUTE_BASE_PATH` / `DIKAROUTE_BASE_PATH`
 */
export function resolveDisplayBaseUrl(
  ...
  // Example: NEXT_PUBLIC_BASE_URL=https://host/dikaroute must not be collapsed to
  ...
  // reverse-proxy subpaths (DIKAROUTE_BASE_PATH).
 ...
 *   1. Public browser origin (+ DIKAROUTE_BASE_PATH when set) — proves the
 ...
 * `src/shared/utils/resolveDikaRouteBaseUrl.ts` instead.
 */
