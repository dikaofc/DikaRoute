/**
 * Client/server helpers for Next.js `basePath` / `DIKAROUTE_BASE_PATH` deploys.
 *
 ...
 * Set via next.config `env.NEXT_PUBLIC_DIKAROUTE_BASE_PATH` from `DIKAROUTE_BASE_PATH`.
 */
export function getDeployBasePath(
  env: NodeJS.ProcessEnv = typeof process !== "undefined" ? process.env : ({} as NodeJS.ProcessEnv)
): string {
  return normalizeBasePath(
    env.NEXT_PUBLIC_DIKAROUTE_BASE_PATH || env.DIKAROUTE_BASE_PATH || ""
  );
}
 ...
 *
 * - Relative absolute paths: `/api/health/ping` → `/dikaroute/api/health/ping`
 * - Absolute same-origin URLs: `https://host/api/x` → `https://host/dikaroute/api/x`
