 ...
 *
 * When a request carries `x-dikaroute-compression`, docs promise the response echoes
 * `X-DikaRoute-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-dikaroute-compression";
const COMPRESSION_RESPONSE_HEADER = DIKAROUTE_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-DikaRoute-Compression: <mode>; source=request-header`
 * when the request supplied `x-dikaroute-compression` and the inner pipeline did not
