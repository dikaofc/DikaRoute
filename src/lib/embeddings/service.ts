import { handleEmbedding } from "@dikaroute/open-sse/handlers/embeddings.ts";
import {
  parseEmbeddingModel,
...
} from "@dikaroute/open-sse/config/embeddingRegistry.ts";
import { errorResponse, unavailableResponse } from "@dikaroute/open-sse/utils/error.ts";
...
import { handleComboChat } from "@dikaroute/open-sse/services/combo.ts";
...
import { attachDikaRouteMetaHeaders } from "@/domain/dikarouteResponseMeta";
...
export async function createEmbeddingResponse(
  ...
  if (result.success) {
    ...
    attachDikaRouteMetaHeaders(responseHeaders, {
      provider,
