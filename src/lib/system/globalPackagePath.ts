...
/**
 * Resolve the real install directory of the globally-installed `dikaroute` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/dikaroute/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/dikaroute`, not `/dikaroute/app`. We probe the real
 ...
 * global install.
 */
export async function resolveGlobalDikaroutePath(
  ...
  const packageRoot = path.join(globalRoot, "dikaroute");
