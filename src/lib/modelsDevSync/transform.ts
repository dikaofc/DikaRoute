 ...
 * types, the provider-id mapping table, and the raw→DikaRoute transform
...
// ─── Provider mapping: models.dev provider ID → DikaRoute provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// DikaRoute uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL DikaRoute identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But DikaRoute's catalog & combo targets
  ...
  // Additional providers that may overlap with DikaRoute
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to DikaRoute provider IDs.
...
/**
 * Transform models.dev raw data → DikaRoute PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as DikaRoute format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const dikaProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped DikaRoute providers
      for (const dikaProvider of dikaProviders) {
        if (!result[dikaProvider]) result[dikaProvider] = {};
        result[dikaProvider][modelId] = entry;
      }
    }
  }
...
export function transformModelsDevToCapabilities(raw: ModelsDevData): CapabilitiesByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      for (const dikaProvider of dikaProviders) {
        if (!result[dikaProvider]) result[dikaProvider] = {};
        result[dikaProvider][modelId] = cap;
      }
    }
  }
