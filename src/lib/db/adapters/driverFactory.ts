...
declare global {
  var __dikarouteSqlJsAdapters: Map<string, SqliteAdapter> | undefined;
  var __dikarouteSqlJsInitPromises: Map<string, Promise<SqliteAdapter>> | undefined;
  var __dikarouteSqlJsPreInitErrors: Map<string, string> | undefined;
}

function getSqlJsCache(): Map<string, SqliteAdapter> {
  if (!globalThis.__dikarouteSqlJsAdapters) {
    globalThis.__dikarouteSqlJsAdapters = new Map();
  }
  return globalThis.__dikarouteSqlJsAdapters;
}

function getSqlJsPreInitErrorCache(): Map<string, string> {
  if (!globalThis.__dikarouteSqlJsPreInitErrors) {
    globalThis.__dikarouteSqlJsPreInitErrors = new Map();
  }
  return globalThis.__dikarouteSqlJsPreInitErrors;
}
...
function getSqlJsPendingCache(): Map<string, Promise<SqliteAdapter>> {
  if (!globalThis.__dikarouteSqlJsInitPromises) {
    globalThis.__dikarouteSqlJsInitPromises = new Map();
  }
  return globalThis.__dikarouteSqlJsInitPromises;
}
...
export function isPackBootForcedSqlJsSmoke(env: NodeJS.ProcessEnv): boolean {
  return env.DIKAROUTE_PACK_BOOT_SMOKE === "1" && env.DIKAROUTE_PACK_BOOT_FORCE_SQLJS === "1";
}
