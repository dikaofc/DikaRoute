...
export function getOmpCredentials(providerId: string) {
  ...
  if (!Database) return { hasDikaRoute: false, baseUrl: null, apiKey: null };
  ...
  try {
    ...
    if (row?.data) {
      ...
      return { hasDikaRoute: true, baseUrl: parsed.baseUrl || null, apiKey: parsed.apiKey || null };
    }
    return { hasDikaRoute: false, baseUrl: null, apiKey: null };
  } catch {
    return { hasDikaRoute: false, baseUrl: null, apiKey: null };
  }
}
