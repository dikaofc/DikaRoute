...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.DIKAROUTE_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal DikaRoute operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`DIKAROUTE_API_KEY` / `ROUTER_API_KEY`)
