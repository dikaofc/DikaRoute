...
export async function getSettings() {
  ...
  const settings: Record<string, unknown> = {
    ...
    preferClaudeCodeForUnprefixedClaudeModels: isTruthyEnvFlag(
      process.env.DIKAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS
    ),
...
export async function resolveProxyForConnection(
  ...
  try {
    const { selectWorkingProxyFallback } = await import("@dikaroute/open-sse/utils/proxyFallback");
