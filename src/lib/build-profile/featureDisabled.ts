/**
 * Helper used by the `DIKAROUTE_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (DIKAROUTE_BUILD_PROFILE=minimal). ` +
        `Install the full dikaroute artifact instead of dikaroute-secure if you need this feature.`
    );
