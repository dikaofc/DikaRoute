...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API DikaRoute
...
export const API_KEY_PLACEHOLDER = "$DIKAROUTE_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export DIKAROUTE_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["DIKAROUTE_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
