...
import { cooldownUntilMs } from "@dikaroute/open-sse/services/accountFallback.ts";
...
declare global {
  var __dikarouteConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__dikarouteConnRecovery) {
    globalThis.__dikarouteConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__dikarouteConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("DIKAROUTE_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("DIKAROUTE_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from DIKAROUTE_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.DIKAROUTE_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
