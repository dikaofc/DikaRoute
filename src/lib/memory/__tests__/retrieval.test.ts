...
describe("Memory Retrieval — FTS5 integration (pre-existing broken test infrastructure)", () => {
  ...
  beforeEach(() => {
    ...
    savedDb = (globalThis as any).__dikarouteDb;
    ...
    setupSchema(db);
    setupFts(db);
    (globalThis as any).__dikarouteDb = db;
  });

  afterEach(() => {
    // Restore the previous singleton (or remove it)
    if (savedDb) {
      (globalThis as any).__dikarouteDb = savedDb;
    } else {
      delete (globalThis as any).__dikarouteDb;
    }
    try {
      db.close();
    } catch {
