...
import { splitClaudeEffortSuffix } from "@dikaroute/open-sse/config/providerModels.ts";
...
export function attachReasoningRuleDirective(
  ...
  body._dikarouteReasoningRouteTrace = {
...
export function applyReasoningRuleDirective(bodyInput: unknown): unknown {
  const source = asRecord(bodyInput);
  const directive = asRecord(source._dikarouteReasoningRule);
  if (!directive.id) return bodyInput;
  const body = { ...source };
  delete body._dikarouteReasoningRule;
