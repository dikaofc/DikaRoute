 ...
 * with the running DikaRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.DIKAROUTE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.DIKAROUTE_TOKEN || process.env.CLI_TOKEN || "",
