...
import {
  ...
  refreshCopilotToken,
} from "@dikaroute/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("DIKAROUTE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * (e.g. "codex,openai"). Targeted alternative to DIKAROUTE_DISABLE_TOKEN_HEALTHCHECK:
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.DIKAROUTE_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
export async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("DIKAROUTE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __dikarouteTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null; sweeping: boolean }
    | undefined;
}
function getHCState() {
  if (!globalThis.__dikarouteTokenHC) {
    globalThis.__dikarouteTokenHC = {
      initialized: false,
      interval: null,
      initTimeout: null,
      sweeping: false,
  ...
  return globalThis.__dikarouteTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in DikaRoute
