...
function isAutoSyncEnabled() {
  // Opt-in, default OFF. Backed by the DIKAROUTE_AUTO_SYNC_CLAUDE_PROFILES feature flag
  ...
  return isFeatureFlagEnabled("DIKAROUTE_AUTO_SYNC_CLAUDE_PROFILES");
}
...
export async function autoSyncClaudeProfilesFromLiveCatalog(
  ...
  // Each generated profile points ANTHROPIC_BASE_URL at the DikaRoute this server serves.
  // Strip a trailing /v1 (Claude Code appends the version segment itself).
  const profileBaseUrl = internalBase.replace(/\/v1$/, "");

  // Reuse the CLI generator so automatic sync and `dikaroute setup-claude` stay
