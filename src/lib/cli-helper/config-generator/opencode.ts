 ...
 * source is the user's OWN DikaRoute instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from DikaRoute. The catalog is the
 ...
 */
export async function fetchDikaRouteCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`DikaRoute /v1/models returned ${response.status} ${response.statusText}`);
    }
...
/**
 * Generate a full `opencode.json` document for DikaRoute. The catalog is the
...
export async function generateOpencodeConfig(options: GenerateOpencodeOptions): Promise<string> {
  ...
  const providerId = options.providerId?.trim() || "dikaroute";
  ...
  if (fetchCatalog) {
    const result = await fetchDikaRouteCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "DikaRoute",
