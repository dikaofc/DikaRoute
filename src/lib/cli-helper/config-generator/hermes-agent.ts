 ...
 *   model: string;                    // the model name the user chose from DikaRoute
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the DikaRoute base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_DIKAROUTE_API_KEY_HERE";
  ...
  // Build the providers.dikaroute entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const dikarouteProvider = {
