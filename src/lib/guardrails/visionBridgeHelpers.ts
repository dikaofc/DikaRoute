 ...
 */
import { detectMediaParts, type MediaPart } from "@dikaroute/open-sse/utils/mediaParts";
 ...
 *      DikaRoute itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      DikaRoute's own `/v1` so the vision model can be any provider
 *      registered in DikaRoute (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. DikaRoute self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known DikaRoute-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require DikaRoute self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through DikaRoute's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via DIKAROUTE_PORT / PORT env vars, falling back to the default 20128.
...
/**
 * Unwrap the detailed-log/diagnostics envelope that some DikaRoute paths attach
 ...
 *      DikaRoute's self-loop when the `stream` default kicks in)
 ...
 * DikaRoute's own self-loop and forceStream providers) reply with a `data:`
...
async function readVisionResponseBody(response: Response): Promise<unknown> {
  try {
    ...
    // that some DikaRoute capture paths attach to provider responses.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // DikaRoute itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the DikaRoute self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so DikaRoute can resolve the
      ...
      // When routing through DikaRoute's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_dikaroute as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_dikaroute";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        // Explicit JSON opt-in: without `Accept: application/json` DikaRoute's
      ...
      if (useFullModelId) {
        headers["x-dikaroute-disabled-guardrails"] = "vision-bridge";
        ...
        // `sk_dikaroute` sentinel OR the operator-configured env key), so
        // external clients cannot use it to bypass admission.
        headers["x-dikaroute-admission-bypass"] = "internal";
        // The admission bypass honors the env key when set (REQUIRE_API_KEY=true
        // deployments) and the `sk_dikaroute` sentinel otherwise. Force the same
