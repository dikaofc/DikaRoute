...
function resolveModel(input: RecordedTriageExecutionInput): string {
  const model = configuredString(input.model, "DIKAROUTE_ISSUE_AGENT_MODEL") ?? DEFAULT_MODEL;
  const provider = configuredString(input.provider, "DIKAROUTE_ISSUE_AGENT_PROVIDER");
...
function resolveTimeoutMs(input: RecordedTriageExecutionInput): number {
  const configured = input.timeoutMs ?? Number(process.env.DIKAROUTE_ISSUE_AGENT_TIMEOUT_MS);
  ...
  return [
    {
      role: "system",
      content:
        "You are the DikaRoute Issue Agent. Analyze only the recorded GitHub context and produce a concise, actionable triage response. Do not claim to have accessed external state.",
    },
    {
      role: "user",
      content: [
...
export async function executeRecordedTriageChatCompletion(
  ...
  const routingPolicy = configuredString(
    input.routingPolicy,
    "DIKAROUTE_ISSUE_AGENT_ROUTING_POLICY"
  );
    ...
    const response = await post(
      new Request("http://localhost/api/v1/chat/completions", {
        ...
        headers: {
          ...
          ...(routingPolicy ? { "X-DikaRoute-Mode": routingPolicy } : {}),
        },
        body: JSON.stringify({
