...
function getLHCState() {
  if (!globalThis.__dikarouteLocalHC) {
    globalThis.__dikarouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__dikarouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("DIKAROUTE_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
