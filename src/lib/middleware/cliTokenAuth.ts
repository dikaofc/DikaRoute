...
const HEADER_NAME = "x-dikaroute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local dikaroute CLI.
 * Only accepted from loopback IPs. Disabled via DIKAROUTE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.DIKAROUTE_DISABLE_CLI_TOKEN === "true") return false;
