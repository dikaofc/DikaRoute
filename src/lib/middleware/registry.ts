...
declare global {
  var __dikaroutePreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__dikaroutePreRequestRegistry) {
    globalThis.__dikaroutePreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__dikaroutePreRequestRegistry;
}
...
function compileHookCode(code: string, hookName: string): HookMiddleware {
  ...
  try {
    script = new vm.Script(`(async () => { ${code} })();`, {
      filename: `dikaroute-hook:${hookName}`,
    });
  } catch (err: unknown) {
