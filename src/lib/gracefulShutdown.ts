...
declare global {
  var __dikarouteShutdown:
    { init: boolean; shuttingDown: boolean; activeRequests: number } | undefined;
}

function getShutdownState() {
  if (!globalThis.__dikarouteShutdown) {
    globalThis.__dikarouteShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
  }
  return globalThis.__dikarouteShutdown;
}
...
async function cleanup(): Promise<void> {
  try {
    const [{ closeAuditDb }, { closeDbInstance }, { flushSpendBatchWriter }, { closeLogRotation }] =
      await Promise.all([
        import("@dikaroute/open-sse/mcp-server/audit.ts"),
        import("@/lib/db/core"),
