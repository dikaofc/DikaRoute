/**
 * Cloudflare Worker source emitter for the DikaRoute proxy relay.
 *
 ...
 *
 * The DikaRoute variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----DikaRouteCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// DikaRoute Cloudflare Worker proxy relay — generated at deploy time.
