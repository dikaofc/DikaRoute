/**
 * DikaRoute Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure DikaRoute on behalf of the user,
...
// ── Helper: check if dikaroute CLI is available ──────────────────────────────

function getDikaRouteCliPath(): string | null {
  try {
    const result = execSync("which dikaroute 2>/dev/null || command -v dikaroute 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the DikaRoute codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within DikaRoute.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runDikaRouteCli",
    description:
      "Execute an 'dikaroute' CLI command to configure or query the DikaRoute app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: dikaroute list-keys, dikaroute switch-combo [id], dikaroute set-budget 10, dikaroute set-strategy [id] priority, dikaroute health, dikaroute mcp (starts MCP server), dikaroute db-health, dikaroute reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'dikaroute'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getDikaRouteCliPath();
      if (!cliPath) return "dikaroute CLI not found in PATH. Install DikaRoute first.";
