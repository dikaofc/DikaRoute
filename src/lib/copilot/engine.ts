...
Use \`createCombo\` tool or \`runDikaRouteCli\` to create them.`;
  }
...
DikaRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the DikaRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## DikaRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`dikaroute health\`
- "CLI list-combos" → ejecuta \`dikaroute list-combos\`
- "CLI set-budget 10" → ejecuta \`dikaroute set-budget 10\`

### Conocimiento
- "Cómo funciona DikaRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with DikaRoute.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runDikaRouteCli") {
    const tool = getCopilotTool("runDikaRouteCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runDikaRouteCli", args: intent.args, result }],
    };
  }
