...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push('  -H "Authorization: Bearer $DIKAROUTE_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g dikaroute   # or: npx dikaroute");
  lines.push("dikaroute --version");
  lines.push("```");
  lines.push("");
