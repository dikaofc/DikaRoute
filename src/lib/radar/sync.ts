 ...
 */
const DEFAULT_FEED_BASE_URL = "https://radar.obitoglory.tech";
...
/**
 * Parse & validate the `x-dikaroute-feed-tier` response header.
 *
...
export async function syncRadar(deps: SyncDeps = {}): Promise<SyncStatus> {
  ...
  try {
    ...
    const signature = res.headers.get("x-dikaroute-feed-signature") ?? "";
    ...
    // The `x-dikaroute-feed-tier` header reflects the tier ACTUALLY served
    ...
    const servedTier = parseServedTierHeader(res.headers.get("x-dikaroute-feed-tier")) ?? feed.tier;
