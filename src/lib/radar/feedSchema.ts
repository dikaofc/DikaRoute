/**
 * feedSchema.ts — Zod schema for the DikaRoute Radar feed payload.
 *
...
/**
 * Exported so `sync.ts` can validate the `x-dikaroute-feed-tier` response
...
export const RadarFeedSchema = z.object({
  feed: z.literal("dikaroute-radar"),
  ...
  // the Authorization key, and is surfaced via the `x-dikaroute-feed-tier`
