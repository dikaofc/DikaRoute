 ...
 * Priority: system daemon socket → DikaRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to DikaRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `dikaroute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
