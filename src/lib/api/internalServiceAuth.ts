...
export const INTERNAL_SERVICE_AUTH_HEADER = "x-dikaroute-internal-service-token";

function configuredToken(): string {
  const inlineToken = process.env.DIKAROUTE_INTERNAL_SERVICE_TOKEN?.trim();
  if (inlineToken) return inlineToken;

  const tokenFile = process.env.DIKAROUTE_INTERNAL_SERVICE_TOKEN_FILE?.trim();
  if (!tokenFile) return "";
