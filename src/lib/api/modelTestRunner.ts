...
import { withRateLimit } from "@dikaroute/open-sse/services/rateLimitManager";
import {
  isCreditsExhausted,
  isDailyQuotaExhausted,
} from "@dikaroute/open-sse/services/accountFallback";
import { looksLikeQuotaExhausted } from "@/shared/utils/classify429";

const INTERNAL_ORIGIN = "http://dikaroute.internal";
...
export function buildInternalChatRequest(
  ...
  return new Request(`${INTERNAL_ORIGIN}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-DikaRoute-Compression": "off",
      "X-Request-Id": `model-test-${randomUUID()}`,
      ...(connectionId ? { "X-DikaRoute-Connection": connectionId } : {}),
    },
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        model: fullModelStr,
        query: "What is DikaRoute?",
        documents: [
          "DikaRoute routes AI requests across configured providers.",
