...
import { runWithProxyContext } from "@dikaroute/open-sse/utils/proxyFetch";
import { logger } from "@dikaroute/open-sse/utils/logger";
...
declare global {
  var __dikarouteWarmupScheduler: {
    timer: NodeJS.Timeout | null;
    executing: boolean;
    lastFireMinute: number;
  };
}
const STATE = (globalThis.__dikarouteWarmupScheduler ??= {
  timer: null,
  executing: false,
  lastFireMinute: -1,
});
...
function isEnabled(): boolean {
  const raw = process.env.DIKAROUTE_WARMUP_ENABLED;
  return raw ? TRUE_ENV_VALUES.has(raw.trim().toLowerCase()) : false;
}

function getCron(): string {
  return process.env.DIKAROUTE_WARMUP_CRON || "0 7 * * *";
}

function getConcurrency(): number {
  const raw = process.env.DIKAROUTE_WARMUP_CONCURRENCY;
...
export function startWarmupScheduler(): NodeJS.Timeout | null {
  ...
  if (!isEnabled()) {
    log.info("disabled (DIKAROUTE_WARMUP_ENABLED not set)");
    return null;
  }
  const cron = getCron();
...
async function executeWarmup(): Promise<void> {
  ...
  for (const conn of connections) {
    ...
    targets.push({
      ...
      headers,
      proxyConfig,
      model: process.env.DIKAROUTE_WARMUP_MODEL || "claude-3-5-haiku-20241022",
    });
  }
...
async function getWarmupHeaders(): Promise<Record<string, string>> {
  const { getClaudeCliHeaders } = await import("@dikaroute/open-sse/config/providers/shared");
