...
import { hasDikaRouteQwenCodeConfig } from "@/shared/services/qwenCodeConfig";
import { getRuntimePorts } from "@/lib/runtime/ports";

const { apiPort } = getRuntimePorts();

/**
 * Check if a tool has DikaRoute configured by reading its config file directly.
  ...
  try {
    ...
    if (toolId === "hermes") {
      ...
      const hasDikaRoute =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasDikaRoute ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores DikaRoute config differently
    switch (toolId) {
      case "claude":
        return (config?.env as Record<string, unknown>)?.ANTHROPIC_BASE_URL
          ...
          : "not_configured";
      case "qwen":
        return hasDikaRouteQwenCodeConfig(config) ? "configured" : "not_configured";
      case "droid":
      case "openclaw":
      case "cline":
      case "kilo": {
        // Generic check: look for DikaRoute-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("dikaroute") ||
          configStr.includes("sk_dikaroute") ||
