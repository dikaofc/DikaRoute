...
// ─── Provider mapping: LiteLLM provider → DikaRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → DikaRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (DikaRoute format).
 ...
 */
export function transformToDikaRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to DikaRoute providers
    const litellmProvider = info.litellm_provider || "";
    const dikaProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (dikaProviders) {
      for (const provider of dikaProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToDikaRoute(raw);
