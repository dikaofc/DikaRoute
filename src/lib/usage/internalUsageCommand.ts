...
const LOCAL_USAGE_MODEL = "dikaroute/local-usage";
  ...
  try {
    ...
    return {
      preferredConnectionId:
        ...
        readHeader(request, "x-dikaroute-connection")?.trim() ||
        null,
      preferredProvider: url.searchParams.get("provider")?.trim() || null,
    };
  } catch {
    return {
      preferredConnectionId: readHeader(request, "x-dikaroute-connection")?.trim() || null,
