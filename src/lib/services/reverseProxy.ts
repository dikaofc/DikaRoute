 ...
 * can be iframed inside the DikaRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between DikaRoute and the embedded service.
 ...
 * Prevents DikaRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   DikaRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the DikaRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (DikaRoute controls framing via its own CSP).
