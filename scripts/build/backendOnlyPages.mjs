 ...
 *
 * When `DIKAROUTE_BUILD_BACKEND_ONLY=1` (or `DIKAROUTE_BUILD_PROFILE=backend`) is set,
 * `build-next-isolated.mjs` calls `stubDashboardPages()` BEFORE `next build` and
 * `restoreDashboardPages()` in a `finally` afterward.
 *
 * WHY: DikaRoute embedders that only consume the HTTP API (`/api/*`, `/v1/*`, `/v1beta/*`)
...
export const BACKEND_ONLY_STUB_MARKER =
  "/* dikaroute:backend-only-stub (auto-restored after build) */";
...
/**
 * Strip a leading `"use server"` module directive. Some DikaRoute API Route Handlers
...
export function isBackendOnlyBuild(env = process.env) {
  return env.DIKAROUTE_BUILD_BACKEND_ONLY === "1" || env.DIKAROUTE_BUILD_PROFILE === "backend";
}
