/**
 * Shared policy for DikaRoute npm publish artifact hygiene.
 *
...
export const APP_STAGING_ALLOWED_EXACT_PATHS: string[] = [
  ...
  // this bare entry the prepublish prune deleted it → every `dikaroute` boot of the
  ...
  // this bare entry the prepublish prune (Step 10.7) deletes it → `dikaroute serve`
...
export const PACK_ARTIFACT_ROOT_ALLOWED_EXACT_PATHS: string[] = [
  ...
  // Runtime polyfill eagerly imported by bin/dikaroute.mjs (Node <22 compat);
...
export const PACK_ARTIFACT_REQUIRED_PATHS: string[] = [
  ...
  "bin/cli/program.mjs",
  // Direct imports of bin/dikaroute.mjs — bin/cli/ is only an allowlist PREFIX, so a
  ...
  "bin/nodeRuntimeSupport.mjs",
  "bin/dikaroute.mjs",
  // #7808: aliasResolver + its hook file. bin/dikaroute.mjs imports
 ...
 * `@dikaroute/opencode-provider/`), so a nested `node_modules` inside an allowed
