...
/**
 * DikaRoute — Prepublish Build Script
 *
...
console.log("🔨 DikaRoute — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "dikaroute.ts");
const cliDestFile = join(ROOT, "bin", "dikaroute.mjs");
  ...
  try {
    runBuildTool(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/dikaroute.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/dikaroute.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @dikaroute/opencode-plugin ──────────────
// The plugin ships bundled inside the dikaroute npm package (see root
// package.json "files": ["@dikaroute/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `dikaroute setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@dikaroute", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @dikaroute/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @dikaroute/opencode-plugin bundled to @dikaroute/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @dikaroute/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @dikaroute/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @dikaroute/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @dikaroute/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @dikaroute/opencode-plugin not found in workspace (skipping build)");
}
