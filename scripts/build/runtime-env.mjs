 ...
 * `DIKAROUTE_MEMORY_MB`, mirroring `dikaroute serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * DIKAROUTE_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `DIKAROUTE_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit DIKAROUTE_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `dikaroute serve` may
 ...
 * the user already exported. #5238: `dikaroute serve` used to UNCONDITIONALLY
...
export function withRuntimePortEnv(env, runtimePorts) {
  ...
  return {
    ...env,
    DIKAROUTE_PORT: String(basePort),
    PORT: String(dashboardPort),
    DASHBOARD_PORT: String(dashboardPort),
    API_PORT: String(apiPort),
    HOSTNAME: env.DIKAROUTE_HOSTNAME || "0.0.0.0",
  };
}
