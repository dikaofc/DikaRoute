#!/usr/bin/env node
/**
 * DikaRoute — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.dikaroute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by DikaRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/dikaroute.mjs`.
  //
  ...
  if (needsPersist) {
    merged.DIKAROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "dikaroute-field-encryption-v1", 32)
              const legacySalt = "dikaroute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.dikaroute/server.env or ~/.dikaroute/.env,"
                );
                log(
                  "   or run: dikaroute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
