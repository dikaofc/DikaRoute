/**
 * WebDAV file server handler for DikaRoute.
 *
...
// Key derivation: scryptSync(secret, "dikaroute-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "dikaroute-field-encryption-v1";
const KEY_LENGTH = 32;
...
/**
 * Decrypt a value that may be encrypted with the DikaRoute enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".dikaroute"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.dikaroute path if it already exists (avoid data loss).
  try {
  ...
  // 3) Windows → %APPDATA%/dikaroute.
  if (process.platform === "win32") {
    const appData = process.env.APPDATA || path.join(homeDir, "AppData", "Roaming");
    return path.join(appData, "dikaroute");
  }
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "dikaroute");
  }

  // 5) Default → legacy ~/.dikaroute (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="DikaRoute WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".dikaroute-webdav-tmp-" + Date.now();
  let writeStream;
  try {
