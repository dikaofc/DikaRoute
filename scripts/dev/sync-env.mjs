...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/dikaroute.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.dikaroute/.env to survive across upgrades. This prevents credential loss
  // when upgrading DikaRoute (issue #1622).
  MACHINE_ID_SALT: () => `dikaroute-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/dikaroute.mjs and persisted to
 * ~/.dikaroute/.env to survive across upgrades.
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "dikaroute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "dikaroute");

  return join(homedir(), ".dikaroute");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.dikaroute/server.env, ~/.dikaroute/.env, ` +
