...
//
// Pure, dependency-light helpers for DikaRoute's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `DIKAROUTE_TLS_CERT` and `DIKAROUTE_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.DIKAROUTE_TLS_CERT === "string" ? env.DIKAROUTE_TLS_CERT.trim() : "";
  const keyPath = typeof env?.DIKAROUTE_TLS_KEY === "string" ? env.DIKAROUTE_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[dikaroute][tls] HTTPS not enabled: both DIKAROUTE_TLS_CERT and DIKAROUTE_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[dikaroute][tls] HTTPS not enabled: could not read TLS cert/key ` +
