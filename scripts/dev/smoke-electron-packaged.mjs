...
export const LINUX_EXECUTABLE_NAMES = ["dikaroute-desktop", "dikaroute", "DikaRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "DikaRoute.app", "Contents", "MacOS", "DikaRoute"),
    join(distDir, "mac-arm64", "DikaRoute.app", "Contents", "MacOS", "DikaRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "DikaRoute.exe"),
    join(distDir, "win-x64-unpacked", "DikaRoute.exe"),
    join(distDir, "win-arm64-unpacked", "DikaRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["DikaRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing DikaRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["dikaroute-desktop", "DikaRoute", "dikaroute"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged DikaRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "dikaroute-electron-smoke-")));
  const removeDataDir =
