...
const bootstrapMode = process.env.DIKAROUTE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.DIKAROUTE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "dikaroute-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_DIKAROUTE_E2E_MODE: process.env.NEXT_PUBLIC_DIKAROUTE_E2E_MODE || "1",
  DIKAROUTE_DISABLE_BACKGROUND_SERVICES:
    process.env.DIKAROUTE_DISABLE_BACKGROUND_SERVICES || "true",
  DIKAROUTE_DISABLE_TOKEN_HEALTHCHECK: process.env.DIKAROUTE_DISABLE_TOKEN_HEALTHCHECK || "true",
  DIKAROUTE_DISABLE_LOCAL_HEALTHCHECK: process.env.DIKAROUTE_DISABLE_LOCAL_HEALTHCHECK || "true",
  DIKAROUTE_HIDE_HEALTHCHECK_LOGS: process.env.DIKAROUTE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        DIKAROUTE_E2E_PASSWORD: "",
        DIKAROUTE_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        DIKAROUTE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.DIKAROUTE_USE_TURBOPACK
    ? {
        DIKAROUTE_USE_TURBOPACK: process.env.DIKAROUTE_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.DIKAROUTE_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.DIKAROUTE_PLAYWRIGHT_SKIP_BUILD === "1") return;
