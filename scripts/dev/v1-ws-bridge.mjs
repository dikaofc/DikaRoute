...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://dikaroute.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://dikaroute.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-dikaroute-ws-session-id": this.sessionId,
      "x-dikaroute-ws-request-id": requestId,
    };
...
export function createDikarouteWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createDikarouteWsBridge requires a baseUrl");
  }
