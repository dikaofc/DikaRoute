...
// Opt-in native HTTPS (#5242). Resolved once at boot: when both DIKAROUTE_TLS_CERT
// and DIKAROUTE_TLS_KEY point at readable files we terminate TLS on the same
...
process.env.DIKAROUTE_INTERNAL_SCHEME = tlsOptions ? "https" : "http";
if (tlsOptions) {
  console.log(`[dikaroute][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`);
}

process.env.DIKAROUTE_WS_BRIDGE_SECRET ||= randomUUID();
...
function getProxy(server) {
  ...
  const proxy = createResponsesWsProxy({
    ...
    bridgeSecret: process.env.DIKAROUTE_WS_BRIDGE_SECRET,
  });
...
http.createServer = function createServerWithResponsesWs(...args) {
  ...
  // (#7003). This wrapper is what `dikaroute serve` / Docker / Electron actually
