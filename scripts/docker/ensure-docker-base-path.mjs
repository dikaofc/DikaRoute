...
/**
 * Compares DIKAROUTE_BASE_PATH at container start against BUILD_DIKAROUTE_BASE_PATH
...
const SENTINEL = "BUILD_DIKAROUTE_BASE_PATH";
...
export function ensureDockerBasePath(opts = {}) {
  ...
  const runtime = normalizeBasePath(opts.env?.DIKAROUTE_BASE_PATH ?? process.env.DIKAROUTE_BASE_PATH);
  ...
  if (!runtime && baked) {
    throw new Error(
      `This DikaRoute image was built for subpath ${baked}, but DIKAROUTE_BASE_PATH is unset. ` +
        `Set DIKAROUTE_BASE_PATH=${baked} or rebuild without the build-arg.`
    );
  }
  ...
  try {
    ...
    console.error(
      "[ensure-docker-base-path] Rebuild the image with the same subpath, e.g.\n" +
        "  docker compose build --build-arg DIKAROUTE_BASE_PATH=/dikaroute"
    );
    process.exit(1);
  }
}
