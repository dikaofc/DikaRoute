...
const contentSecurityPolicy = [
  ...
  // DikaRoute is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// DIKAROUTE_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `dikaroute-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.DIKAROUTE_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // DikaRoute under https://host/dikaroute/). Empty by default so root-path
  ...
  basePath: normalizeBasePath(process.env.DIKAROUTE_BASE_PATH),
  ...
  env: {
    NEXT_PUBLIC_DIKAROUTE_BASE_PATH: normalizeBasePath(process.env.DIKAROUTE_BASE_PATH),
  },
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets DIKAROUTE_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.DIKAROUTE_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    // Reduce peak heap during production builds (Next.js 15+).
    webpackMemoryOptimizations: true,
    // DikaRoute: single-process webpack build on memory-constrained hosts.
    // Turbopack (unbounded Rust heap) froze this 7.7GB/4-core box 3x; the
    // split worker stacked two >3GB V8 heaps then SIGABRT. One bounded
    // process caps total V8 heap via DIKAROUTE_BUILD_MEMORY_MB.
    ...
    // Next's internal router proxy defaults to 30s when this is unset. DikaRoute
    ...
    // PR-2 of : tree-shake barrel re-exports so
    ...
    // the internal `@dikaroute/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@dikaroute/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  ...
  webpack(config, { webpack }) {
    ...
    config.optimization.splitChunks = {
      ...
      cacheGroups: {
        ...
        // PR-2 of : isolate the heavy long-tail
  ...
  async headers() {
    return [
      ...
      // G-10: allow DikaRoute's own dashboard to embed the 9Router UI via our reverse proxy.
  ...
  async rewrites() {
    return [
      ...
      // so they hit the /api/[...dikarouteApiCatchAll] route (#6424) — which
