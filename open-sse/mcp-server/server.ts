...
import { resolveDikaRouteBaseUrl } from "../../src/shared/utils/resolveDikaRouteBaseUrl.ts";
import { sanitizeErrorMessage } from "../utils/error.ts";
import { getMcpModelsCatalog } from "./catalog.ts";
export { getMcpModelsCatalog } from "./catalog.ts";

const DIKAROUTE_BASE_URL = resolveDikaRouteBaseUrl();
const MCP_ENFORCE_SCOPES = process.env.DIKAROUTE_MCP_ENFORCE_SCOPES === "true";
const MCP_ALLOWED_SCOPES = new Set(
  (process.env.DIKAROUTE_MCP_SCOPES || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
...
function getDikaRouteApiKey(): string {
  return process.env.DIKAROUTE_API_KEY || "";
}

export async function dikaRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  const url = `${DIKAROUTE_BASE_URL}${path}`;
  const apiKey = getDikaRouteApiKey();
  ...
  if (!response.ok) {
    ...
    throw new Error(`DikaRoute API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    const [healthRaw, resilienceRaw, rateLimitsRaw] = await Promise.allSettled([
      dikaRouteFetch("/api/monitoring/health"),
      dikaRouteFetch("/api/resilience"),
      dikaRouteFetch("/api/rate-limits"),
    ]);
    ...
    await logToolCall("dikaroute_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  const start = Date.now();
  try {
    const combosRaw = await dikaRouteFetch("/api/combos");
    ...
    if (args.includeMetrics) {
      metrics = toRecord(await dikaRouteFetch("/api/combos/metrics").catch(() => ({})));
    }
    ...
    await logToolCall("dikaroute_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  const start = Date.now();
  try {
    const result = await dikaRouteFetch(
      `/api/combos/metrics?comboId=${encodeURIComponent(args.comboId)}`
    );
    await logToolCall("dikaroute_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  const start = Date.now();
  try {
    const result = await dikaRouteFetch(`/api/combos/${encodeURIComponent(args.comboId)}`, {
      method: "PUT",
      body: JSON.stringify({ isActive: args.active }),
    });
    await logToolCall("dikaroute_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCreateCombo(args: {
  ...
  const start = Date.now();
  try {
    const result = await dikaRouteFetch("/api/combos", {
      method: "POST",
      body: JSON.stringify(args),
    });
    await logToolCall("dikaroute_create_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_create_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    const result = normalizeQuotaResponse(await dikaRouteFetch(path), {
      provider: args.provider || null,
      connectionId: args.connectionId || null,
    });

    await logToolCall("dikaroute_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleRouteRequest(args: {
  ...
  try {
    ...
    const raw = (await dikaRouteFetch("/v1/chat/completions", {
      method: "POST",
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    const raw = toRecord(
      await dikaRouteFetch(`/api/usage/analytics?range=${encodeURIComponent(range)}`)
    );
    ...
    await logToolCall("dikaroute_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    const result = await dikaRouteFetch("/v1/search", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("dikaroute_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebFetch(args: {
  ...
  try {
    ...
    const result = await dikaRouteFetch("/v1/web/fetch", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("dikaroute_web_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dikaroute_web_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    "dikaroute_get_health",
    {
      description:
        "Returns DikaRoute health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("dikaroute_get_health", async (args) => {
  ...
  server.registerTool(
    "dikaroute_route_request",
    {
      description: "Sends a chat completion request through DikaRoute intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("dikaroute_route_request", (args) =>
      handleRouteRequest(routeRequestInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_cost_report",
    {
      description: "Generates a cost report for the specified period",
      inputSchema: costReportInput,
    },
    withScopeEnforcement("dikaroute_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("dikaroute_set_resilience_profile", (args) =>
      handleSetResilienceProfile(setResilienceProfileInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_test_combo",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_test_combo", (args) =>
      handleTestCombo(testComboInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("dikaroute_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "dikaroute_db_health_check",
    {
      description:
        "Diagnoses or repairs DikaRoute database drift, including broken combo references and orphan quota/domain rows",
      inputSchema: dbHealthCheckInput,
    },
    withScopeEnforcement("dikaroute_db_health_check", (args) =>
      handleDbHealthCheck(dbHealthCheckInput.parse(args ?? {}))
    )
  );

  server.registerTool(
    "dikaroute_sync_pricing",
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into DikaRoute without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("dikaroute_sync_pricing", (args) =>
      handleSyncPricing(syncPricingInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_web_search",
    {
      description:
        "Performs a web search using DikaRoute's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("dikaroute_web_search", (args) =>
  ...
  server.registerTool(
    ...
    {
      description:
        "Fetches and extracts content from a URL using DikaRoute's web fetch gateway. Supports multiple providers (Firecrawl, Jina Reader, Tavily) with automatic failover. Returns the page content as markdown, HTML, links, or screenshot, along with metadata.",
      inputSchema: webFetchInput,
    },
    withScopeEnforcement("dikaroute_web_fetch", (args) => handleWebFetch(webFetchInput.parse(args)))
  );

  server.registerTool(
    "dikaroute_cache_stats",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_cache_stats", () => handleCacheStats())
  );

  server.registerTool(
    "dikaroute_cache_flush",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_cache_flush", (args) =>
      handleCacheFlush(cacheFlushInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_oneproxy_fetch",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_oneproxy_fetch", (args) =>
      handleOneproxyFetch(oneproxyFetchInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_oneproxy_rotate",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_oneproxy_rotate", (args) =>
      handleOneproxyRotate(oneproxyRotateInput.parse(args))
    )
  );

  server.registerTool(
    "dikaroute_oneproxy_stats",
    {
      description:
    ...
    withScopeEnforcement("dikaroute_oneproxy_stats", (args) =>
 ...
 * Called when `dikaroute --mcp` is used.
 */
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] DikaRoute MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] DikaRoute MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
