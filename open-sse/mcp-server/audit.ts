...
declare global {
  var __dikarouteMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__dikarouteMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__dikarouteMcpAuditDb = database;
}
  ...
  if (!nodeSqliteFallbackAvailable()) {
    console.error(
      `[MCP Audit] better-sqlite3 native binding unavailable and Node ${process.version} ` +
        ...
        "`npm rebuild better-sqlite3` in the dikaroute install root."
    );
    return null;
  }
 ...
 * Uses the same SQLite database as the main DikaRoute app.
 *
 * Driver priority:
...
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".dikaroute", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.DIKAROUTE_API_KEY_ID || null;
