/**
 * DikaRoute MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. dikaroute_compression_status   — Get compression config, analytics, and cache stats
 *   2. dikaroute_compression_configure — Update compression settings
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("dikaroute_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("dikaroute_compression_configure", args, result, duration, true);
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    await logToolCall(
      "dikaroute_compression_configure",
      args,
      { error: errorMessage },
      duration,
      false,
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("dikaroute_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("dikaroute_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("dikaroute_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("dikaroute_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("dikaroute_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("dikaroute_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleRtkDiscover(
  ...
  await logToolCall("dikaroute_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("dikaroute_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
...
export const compressionTools = {
  ...
  dikaroute_set_compression_engine: {
  ...
  dikaroute_list_compression_combos: {
  ...
  dikaroute_compression_combo_stats: {
  ...
  dikaroute_ccr_inspect: {
    name: "dikaroute_ccr_inspect",
  ...
  dikaroute_ccr_delete: {
    name: "dikaroute_ccr_delete",
  ...
  dikaroute_rtk_discover: {
    name: "dikaroute_rtk_discover",
    description:
