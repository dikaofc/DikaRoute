 ...
 * `extra.authInfo` (DikaRoute authenticates with API keys, not OAuth client ids —
 ...
 *
 * On stdio transport (`dikaroute --mcp`) there is no HTTP context, so we fall
 * back to the DIKAROUTE_API_KEY / ROUTER_API_KEY environment variable.
 *
 ...
 *
 * Falls back to DIKAROUTE_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the DIKAROUTE_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.DIKAROUTE_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
