...
// mock through the audit connection cache (globalThis.__dikarouteMcpAuditDb),
...
describe("MCP audit shutdown", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    globalThis.__dikarouteMcpAuditDb = undefined;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dikaroute-mcp-audit-"));
  ...
  it(
    ...
    async () => {
      ...
      globalThis.__dikarouteMcpAuditDb = mockDb as unknown as typeof globalThis.__dikarouteMcpAuditDb;

      await audit.logToolCall("dikaroute_get_health", { ok: true }, { ok: true }, 12, true);
  ...
  it("still closes the audit database when checkpoint fails", async () => {
    ...
    await audit.logToolCall("dikaroute_get_health", {}, {}, 5, true);
    expect(audit.closeAuditDb()).toBe(true);
    expect(mockDb.close).toHaveBeenCalledTimes(1);
  });
  ...
  it("falls back to node:sqlite when better-sqlite3 binding is missing", async () => {
    ...
    try {
      await audit.logToolCall("dikaroute_get_health", { ok: true }, { ok: true }, 4, true);
