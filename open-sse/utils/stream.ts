...
import { buildDikaRouteSseMetadataComment } from "@/domain/dikarouteResponseMeta";
import {
...
export const PENDING_REQUEST_CLEARED_MARKER = "__dikaroutePendingRequestCleared";
...
export function createSSEStream(options: StreamOptions = {}) {
  ...
  const emitFinalSseMetadata = async (
    ...
    const comment = buildDikaRouteSseMetadataComment({
      provider,
      model,
      cacheHit: false,
            ...
            if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
              try {
                ...
                if (isResponsesSSE) {
                  ...
                  // chunk like "[DikaRoute] Upstream returned an empty response. Please
