...
/**
 * Whether DikaRoute may emit SSE `:` comment lines (e.g. the `: keepalive` heartbeat).
 * Some strict OpenAI-compatible clients parse every SSE line as JSON and crash on `:` comments.
 * Set DIKAROUTE_SSE_COMMENTS=off to suppress comment-shaped heartbeats (they become a no-op).
...
export function sseCommentsEnabled(): boolean {
  ...
  const v = process.env.DIKAROUTE_SSE_COMMENTS;
...
export function createSseHeartbeatTransform({
  ...
  // crash on `:` comment heartbeats. DIKAROUTE_SSE_COMMENTS=off disables comment-shaped
