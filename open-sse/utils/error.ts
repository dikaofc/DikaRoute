 ...
 * `x-dikaroute-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-dikaroute-recovery-action` /
 * `x-dikaroute-recovery-next-step` / `x-dikaroute-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-dikaroute-combo-pool-size": String(safe.poolSize),
    "x-dikaroute-combo-attempted": String(safe.attempted),
    "x-dikaroute-combo-excluded": excludedHeader,
    "x-dikaroute-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-dikaroute-recovery-action"] = safe.recovery.action;
    ...
    headers["x-dikaroute-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-dikaroute-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "Retry-After": String(retryAfterSec),
        "X-DikaRoute-Provider-Breaker": "open",
      },
    }
  );
}
