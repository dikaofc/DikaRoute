 ...
 * `stream`. Without this hint, DikaRoute previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // DikaRoute accumulates the provider stream and converts it to JSON for the client
  ...
  // DikaRoute fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve DikaRoute's streaming default unless
