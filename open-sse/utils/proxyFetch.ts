...
// Overridable via DIKAROUTE_RELAY_FETCH_TIMEOUT_MS (capped at 29s so the
// relay-specific timeout always fires first).
function readRelayFetchTimeoutMs(): number {
  const raw = process.env.DIKAROUTE_RELAY_FETCH_TIMEOUT_MS;
  if (raw == null || raw.trim() === "") return 25_000;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    console.warn(
      `[ProxyFetch] Invalid DIKAROUTE_RELAY_FETCH_TIMEOUT_MS="${raw}". Using default 25000.`
    );
    return 25_000;
  }
...
// Overridable via DIKAROUTE_RETRY_BACKOFF_MS (0 = retry immediately).
const RETRY_BACKOFF_MS = Math.max(Number(process.env.DIKAROUTE_RETRY_BACKOFF_MS) || 10, 0);
...
const PATCH_STATE_KEY = Symbol.for("dikaroute.proxyFetch.state");
...
export async function runWithProxyContext(
  ...
  return proxyContext.run(effectiveProxyConfig, async () => {
    if (resolvedProxyUrl && effectiveProxyConfig !== currentContext) {
      ...
      if (process.env.DIKAROUTE_PROXY_FETCH_DEBUG === "true") {
        console.log(
 ...
 *
 * This remains disabled unless DIKAROUTE_CONTROL_PLANE_PROXY_DIRECT_FALLBACK is enabled
      ...
      try {
        ...
        if (
          ...
          if (attempt === 0 && maxAttempts > 1) {
            ...
            // Delay is DIKAROUTE_RETRY_BACKOFF_MS (default 10ms): a fixed backoff
    ...
    for (let attempt = 0; attempt < maxRelayAttempts; attempt++) {
      ...
      try {
        ...
        if (attempt === 0 && maxRelayAttempts > 1 && isTransportFailure) {
          ...
          // #9158: fixed DIKAROUTE_RETRY_BACKOFF_MS backoff — the retry uses a
