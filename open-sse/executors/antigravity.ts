...
function sanitizeAntigravityGeminiRequest(
  ...
  // Missing settings stay absent so DikaRoute does not silently weaken upstream safety.
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  async transformRequest(
    ...
    // Opt-in escape hatch: set DIKAROUTER_ALLOW_BODY_PROJECT_OVERRIDE=1.
    ...
    const allowBodyProjectOverride = process.env.DIKAROUTE_ALLOW_BODY_PROJECT_OVERRIDE === "1";
    ...
    // Opt-in escape hatch: set DIKAROUTE_ALLOW_BODY_PROJECT_OVERRIDE=1.
    let projectId =
