...
/**
 * Strip the DikaRoute provider prefix from tool model fields (e.g.
 ...
 * DikaRoute prefix makes Anthropic reject the request.
 *
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip DikaRoute provider prefix from versioned built-in tool
          ...
          //   x-dikaroute-effort:   low | medium | high | xhigh | max | off
          //   x-dikaroute-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-dikaroute-effort"] ?? clientHeaders?.["X-DikaRoute-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-dikaroute-thinking"] ?? clientHeaders?.["X-DikaRoute-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-dikaroute-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one DikaRoute.
          ...
          // is pinned to the captured CLI wire image, not DikaRoute's Node.
