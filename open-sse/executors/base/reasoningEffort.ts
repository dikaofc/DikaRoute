...
/**
 * Map DikaRoute's reasoning-effort inputs onto the binary thinking switch exposed by
...
export function supportsMaxEffortForProvider(provider: string, model: string): boolean {
  ...
  // normalized to xhigh (the DikaRoute-internal top tier) and rejected by the
...
// DikaRoute carries the requested effort on up to three shapes:
//   1. top-level `reasoning_effort`        — OpenAI / DikaRoute-internal
...
export function sanitizeReasoningEffortForProvider(
  ...
  // ONLY as {high, max} (its own top tier is literally "max"). DikaRoute's internal
