...
function shouldUseBrowserBacked(): boolean {
  ...
  const poolFlag = process.env.DIKAROUTE_BROWSER_POOL;
 ...
 * retired id DikaRoute historically advertised to the current wire id served by
...
export class DuckDuckGoWebExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    // Browser-backed path: opt-in via DIKAROUTE_BROWSER_POOL=on or
