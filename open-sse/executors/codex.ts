...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._dikarouteResponsesStore;
  delete body._dikarouteResponsesStore;
  return marker;
}

/**
 * Global Codex WebSocket kill-switch (feature flag DIKAROUTE_CODEX_WS_ENABLED,
...
function isCodexWsGloballyEnabled(): boolean {
  try {
    return isFeatureFlagEnabled("DIKAROUTE_CODEX_WS_ENABLED");
  } catch {
    return true;
  }
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // DikaRoute is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
function codexDropNonstandardEvents(): boolean {
  const v = process.env.DIKAROUTE_CODEX_DROP_NONSTANDARD_EVENTS;
...
// streaming Response when the DIKAROUTE_CODEX_DROP_NONSTANDARD_EVENTS kill-switch is on.
...
export function encodeResponseSseEvent(raw: string): { sse: string; terminal: boolean } {
  ...
  // DIKAROUTE_CODEX_DROP_NONSTANDARD_EVENTS (the HTTP transport is handled
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by DikaRoute's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by DikaRoute pipeline
      "_dikarouteResponsesStore",
    ]);
