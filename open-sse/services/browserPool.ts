 ...
 * startup cost. Set DIKAROUTE_BROWSER_POOL=off to fully disable.
 */
...
// (and the dikaroute_browser_pool_status MCP tool), giving the previously
...
function isPoolEnabled(): boolean {
  const flag = process.env.DIKAROUTE_BROWSER_POOL;
...
export async function acquireBrowserContext(
  ...
  if (!isPoolEnabled()) {
    throw new Error(
      "browserPool: DIKAROUTE_BROWSER_POOL=off — context requested but pool is disabled"
    );
  }
 ...
 * failures, shutdowns). Surfaced via the dikaroute_browser_pool_status MCP tool.
 */
