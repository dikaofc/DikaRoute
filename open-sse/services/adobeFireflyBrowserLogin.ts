...
export function resolveSystemBrowserExecutable(): string | null {
  const configured = process.env.DIKAROUTE_LOGIN_BROWSER_PATH?.trim();
...
export async function startAdobeFireflyBrowserLogin(
  ...
  if (!browserPath) {
    return {
      ...
      error:
        ...
        "Install Google Chrome or Microsoft Edge, or set DIKAROUTE_LOGIN_BROWSER_PATH, " +
  ...
  try {
    userDataDir = mkdtempSync(join(tmpdir(), "dikaroute-firefly-login-"));
