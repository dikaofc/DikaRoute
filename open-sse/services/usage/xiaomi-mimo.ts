 ...
 * cannot reach the `tokenPlan/usage` endpoint), so DikaRoute self-tracks the
 ...
 * API to call. Instead we count the tokens DikaRoute itself routed to this
 ...
 * through DikaRoute, not the provider's own dashboard figure.
 */
export async function getXiaomiMimoUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "Xiaomi MiMo Token Plan (DikaRoute-tracked)",
      quotas: {
