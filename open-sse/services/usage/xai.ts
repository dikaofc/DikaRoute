 ...
 * self-track pattern — DikaRoute sums the tokens it itself routed to this
 ...
 * MiMo self-track pattern above — DikaRoute sums the tokens it itself routed
...
export async function getXaiUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "xAI / Grok (DikaRoute-tracked)",
      quotas: {
        monthly: {
          used,
          total: 0,
