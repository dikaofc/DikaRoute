 ...
 * with the connection's OAuth access token, falling back to DikaRoute's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · DikaRoute-tracked",
      message: "Live weekly quota unavailable; showing DikaRoute-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
