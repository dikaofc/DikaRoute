 ...
 * DikaRoute self-tracks the USD it spent through the connection (summed from
 ...
 * the USD that DikaRoute has spent through this connection since the account was added — summed
...
export async function getVertexUsage(connectionId: string, provider: string) {
  ...
  try {
    ...
    if (requests === 0) {
      return {
        plan: "Vertex AI",
        message: "Vertex connected. No usage recorded through DikaRoute yet for this account.",
        quotas: { spend },
      };
    }
