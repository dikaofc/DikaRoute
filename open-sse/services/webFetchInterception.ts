 ...
 * synthetic `dikaroute_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export const DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME = "dikaroute_web_fetch";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: DIKAROUTE_WEB_FETCH_FALLBACK_TOOL_NAME,
