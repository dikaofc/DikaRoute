 ...
 * not reliable here because DikaRoute may force a streaming envelope internally.
 */
 ...
 *
 * Supports BOTH wire formats DikaRoute may emit:
 ...
 * `x-dikaroute-chaos-error` header and an error final chunk (status stays 200
...
export async function handleChaosChat(opts: {
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-DikaRoute-Chaos": "true",
      "X-DikaRoute-Chaos-Panel": String(panel.length),
      "X-DikaRoute-Chaos-Primary": primaryModel ?? "",
    },
  });
}
