 ...
 *
 *   X-DikaRoute-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-DikaRoute-Budget:          <max USD per request>                             (#6023)
 *   X-DikaRoute-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-DikaRoute-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-DikaRoute-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-DikaRoute-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-dikaroute-mode")?.trim() || null;
  const budgetHeader = headers.get("x-dikaroute-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-dikaroute-budget-fallback")?.trim() || null;
