 ...
 *
 * Goal: ensure the final request body DikaRoute sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - dikaroute-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "dikaroute-daystamp";
  /**
 ...
 * proven against the live DikaRoute deployment (call log
...
/**
 * Compute the `cc_version` suffix per the DikaRoute native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "dikaroute-daystamp"
