...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-DikaRoute-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an DikaRoute
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-dikaroute-provider-breaker");
 ...
 *   service supervisor outage signalled via `X-Dikaroute-Fallback-Hint: connection_cooldown`)
