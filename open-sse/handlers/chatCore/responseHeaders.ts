import {
  attachDikaRouteMetaHeaders,
  buildDikaRouteResponseMetaHeaders,
} from "@/domain/dikarouteResponseMeta";
import { DIKAROUTE_RESPONSE_HEADERS } from "@/shared/constants/headers";
import { defaultLogger } from "@dikaroute/open-sse/utils/logger";
 ...
 * (typically `process.env.DIKAROUTE_FORWARDING_HEADER_BUDGET_BYTES`). Returns the
...
export function resolveForwardedHeaderBudget(env?: string): number {
  const parsed = Number.parseInt(String(env ?? process.env.DIKAROUTE_FORWARDING_HEADER_BUDGET_BYTES), 10);
 ...
 * This budget includes each header name, separator, value, and trailing CRLF. DikaRoute's own
 * response metadata and framework/security headers are added separately.
 * Override with `DIKAROUTE_FORWARDING_HEADER_BUDGET_BYTES`.
 */
...
function isDikaRouteInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-dikaroute-");
}
 ...
 *
 * If DikaRoute re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isDikaRouteInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
  ...
  attachDikaRouteMetaHeaders(responseHeaders, meta);
