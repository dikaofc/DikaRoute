 ...
 * `x-dikaroute-no-memory` header. Mirrors the existing `x-dikaroute-no-cache`
...
export function isNoMemoryRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dikaroute-no-memory") || "")
    .trim()
    .toLowerCase();
...
/**
 * Per-request compression override via the `x-dikaroute-compression` header. Mirrors the
 * `x-dikaroute-no-memory` convention (#4290). Returns the raw trimmed value, or null when
...
export function resolveCompressionHeader(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dikaroute-compression") || "").trim();
  return value || null;
}

/**
 * Per-request opt-in to unconditionally strip `reasoning_content` from the
 * non-streaming JSON response via the `x-dikaroute-strip-reasoning` header.
...
export function isStripReasoningRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dikaroute-strip-reasoning") || "")
    .trim()
    .toLowerCase();
  return value === "true" || value === "1" || value === "yes";
}
