 ...
 * shapes), and attach an `_dikaroute` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _dikaroute: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._dikaroute &&
    typeof payload._dikaroute === "object" &&
    !Array.isArray(payload._dikaroute)
      ? payload._dikaroute
      : {};
  return {
    ...payload,
