export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-DikaRoute-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.DIKAROUTE_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.DIKAROUTE_PUBLIC_PROTOCOL || "http";
