...
// Preserve DikaRoute's existing 2-minute cap to avoid changing runtime behavior.
...
/**
 * G-02: detect embedded service supervisor failures (X-Dikaroute-Fallback-Hint: connection_cooldown).
...
export function serviceSupervisorCooldown(
  ...
  const hintValue =
    typeof (headers as Headers).get === "function"
      ? (headers as Headers).get("x-dikaroute-fallback-hint")
      : (headers as Record<string, string>)["x-dikaroute-fallback-hint"] ||
        (headers as Record<string, string>)["X-Dikaroute-Fallback-Hint"];
