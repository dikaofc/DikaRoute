 ...
 *
 * Problem (#7791): when DikaRoute is installed via `npm i -g dikaroute`, the
 * package files live under `node_modules/dikaroute/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@dikaroute/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *   - `@/*`             → `./src/*`
 *   - `@dikaroute/open-sse`    → `./open-sse/index.ts`
 *   - `@dikaroute/open-sse/*`  → `./open-sse/*`
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@dikaroute/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@dikaroute/open-sse` →
...
export const ALIAS_MAP = [
  ...
  { prefix: "@dikaroute/open-sse/", target: "open-sse", exact: false },
  { prefix: "@dikaroute/open-sse", target: "open-sse", exact: true },
];
 ...
 *   "@/..."                     → <root>/src/...
 *   "@dikaroute/open-sse/..."   → <root>/open-sse/...
 *   "@dikaroute/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@dikaroute/open-sse` with no trailing path) →
