...
// ahead of expensive self-heal hooks), adapted to DikaRoute's Commander CLI where the
...
// #7791: tsx's tsconfig-path resolution does not apply when DikaRoute is
// installed globally (files live under node_modules/dikaroute/), so bare
...
function loadEnvFile() {
  ...
  if (process.env.DIKAROUTE_CLI_SKIP_REPO_ENV !== "1") {
...
// Generate STORAGE_ENCRYPTION_KEY if not set (persisted to ~/.dikaroute/.env)
// This ensures the key survives across upgrades and is not regenerated on each install.
// See: https://github.com/dikaofc/DikaRoute/issues/1622
//
...
// key or write ~/.dikaroute/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.dikaroute (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".dikaroute");
    const envPath = join(dataDir, ".env");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.DIKAROUTE_LANG;
...
process.on("exit", () => {
  if (process.env.DIKAROUTE_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g dikaroute` or `dikaroute update --apply`",
    });
  }
});
