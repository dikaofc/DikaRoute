# DikaRoute CLI — Internal Conventions
...
```
dikaroute keys add openai sk-xxx
dikaroute combo switch fastest
dikaroute memory search "react hooks"
```

**Not allowed**:

```
dikaroute --add-key openai sk-xxx     # ❌ flag-as-verb
dikaroute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `DIKAROUTE_BASE_URL` env or `~/.dikaroute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${DIKAROUTE_API_KEY}` when available.
- Injects `x-dikaroute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `DIKAROUTE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `DIKAROUTE_LANG` to `~/.dikaroute/.env`.
