# bin/cli — DikaRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `dikaroute` binary.
...
├── sqlite.mjs              ← openDikaRouteDb() — DB bootstrap
...
All HTTP calls to the DikaRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `DIKAROUTE_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `DIKAROUTE_API_KEY`)
...
Locale detection order: `DIKAROUTE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `DIKAROUTE_LANG` environment variable
...
```bash
dikaroute config lang set pt-BR       # saves to ~/.dikaroute/.env
dikaroute config lang list            # show all 42 available locales
dikaroute config lang get             # show currently active locale
```

**One-time override:**

```bash
dikaroute --lang de providers list    # run in German, not persisted
DIKAROUTE_LANG=ja dikaroute status    # same effect via env
```
