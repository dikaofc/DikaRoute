...
const APP_LABEL = "com.dikaroute.autostart";
const WIN_REG_VALUE = "DikaRoute";
const WIN_STARTUP_FILE = "DikaRoute.vbs";
const LINUX_SERVICE_NAME = "dikaroute.service";
const LINUX_DESKTOP_NAME = "dikaroute.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v dikaroute 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "dikaroute.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("dikaroute.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".dikaroute", ".env");
  const lines = [
    "[Unit]",
    "Description=DikaRoute AI proxy router",
 ...
 * running process. When the running DikaRoute cli was itself spawned by the
...
/**
 * Builds the VBScript source that launches DikaRoute with WSH's Run method
