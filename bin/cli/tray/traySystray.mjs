...
// ~/.dikaroute/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `DikaRoute  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit DikaRoute", enabled: true },
  ];
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[dikaroute][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
