...
// prebuilt addon, etc.), every `dikaroute doctor` DB check reported a false FAIL
  ...
  if (message.includes("NODE_MODULE_VERSION") || message.includes("ERR_DLOPEN_FAILED")) {
    return new Error(
      "better-sqlite3 native binding is incompatible with this Node.js runtime. " +
        "Run `npm rebuild better-sqlite3` in the DikaRoute project and try again. " +
        "Or run: dikaroute runtime repair  " +
...
export async function openDikaRouteDb() {
...
export async function readDatabaseHealth(dbPath) {
  return withReadonlySqlite(dbPath, (db) => {
    ...
    const hasMigrationTable = !!db
      ...
      .get("_dikaroute_migrations");
    const appliedMigrationVersions = hasMigrationTable
      ? db
          .prepare("SELECT version FROM _dikaroute_migrations")
          .all()
          .map((row) => row.version)
      : [];
