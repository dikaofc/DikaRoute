/**
 * dikaroute setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running DikaRoute instance (local or remote VPS), fetches the
 ...
 *   dikaroute setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  const apiKey = opts.apiKey ?? opts["api-key"] ?? process.env.DIKAROUTE_API_KEY ?? "";
  const codexHome = opts.codexHome ?? opts["codex-home"] ?? join(os.homedir(), ".codex");
  const dryRun = Boolean(opts.dryRun ?? opts["dry-run"]);
  const onlyFilter = opts.only ? opts.only.split(",").map((s) => s.trim()) : null;

  printHeading(`DikaRoute → Codex CLI profile generator`);
  ...
  try {
    ...
    printInfo(
      "Make sure DikaRoute is running and the --remote URL is correct.\n" +
        "You may also need --api-key if DikaRoute requires authentication."
    );
    return 1;
  }
...
export function registerSetupCodex(program) {
  program
    .command("setup-codex")
    .description(
      "Fetch the live model catalog from DikaRoute (local or remote VPS) and generate " +
    ...
    .option(
      "--remote <url>",
      "Remote DikaRoute URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "DikaRoute API key for the remote instance (defaults to DIKAROUTE_API_KEY env var)"
    )
