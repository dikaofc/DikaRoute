...
/**
 * `dikaroute connect <host>` — remote mode.
 *
 * Logs into a remote DikaRoute server and saves the result as the active context
...
export async function runConnectCommand(host, opts = {}) {
  ...
  if (!baseUrl) {
    printError("A host is required, e.g. dikaroute connect 192.168.0.15");
    return 2;
  }
  ...
  cfg.contexts[name] = {
    ...
    accessToken,
    scope,
    description: `Remote DikaRoute (${host})`,
  };
  ...
  printInfo("Switch back to local with: dikaroute contexts use default");
  return 0;
}
...
export function registerConnect(program) {
  program
    ...
    .description(
      t("connect.description") || "Connect to a remote DikaRoute server and enter remote mode"
    )
