/**
 * dikaroute setup-kilo — configure Kilo Code to use DikaRoute.
 *
 ...
 * This writes BOTH (matching the DikaRoute dashboard) and prints the UI settings.
 *
...
/** Merge the DikaRoute openai-compatible provider into Kilo's CLI auth.json. */
export function buildKiloAuth(existing, { apiKey, baseUrl, model }) {
  const auth = { ...(existing || {}) };
  auth["openai-compatible"] = {
    ...(auth["openai-compatible"] || {}),
    apiKey: apiKey || "sk_dikaroute",
    baseUrl,
    model,
  };
  return auth;
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "DikaRoute", baseURL: baseUrl, apiKey: apiKey || "sk_dikaroute" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_dikaroute" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for DikaRoute: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
