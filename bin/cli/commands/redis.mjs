...
const DEFAULT_NAME = "dikaroute-redis";
const DEFAULT_PORT = "6379";
const DEFAULT_VOLUME = "dikaroute-redis-data";
...
export function registerRedis(program) {
  const redis = program
    ...
    .description(
      t("redis.description") ||
        "Launch a 1-click local Redis container (Podman or Docker) for DikaRoute caching and quota tracking"
    );
...
export async function runRedisUpCommand(opts = {}) {
  ...
  try {
    ...
    info(`Set DIKAROUTE_REDIS_URL=redis://${bind}:${port} in your .env to wire DikaRoute to it.`);
...
export async function runRedisStatusCommand(opts = {}) {
  ...
  if (!running) {
    info(`Run 'dikaroute redis up' to launch it.`);
  }
  return 0;
}
