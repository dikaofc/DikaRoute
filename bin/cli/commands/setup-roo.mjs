/**
 * dikaroute setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for DikaRoute.
 *
 ...
 * must support native OpenAI tool-calling (DikaRoute does).
 */
...
export function resolveRooTarget(opts = {}) {
  ...
  if (opts.remote) root = String(opts.remote).replace(/\/+$/, "");
    ...
    try {
      root = resolveActiveContext(opts.context ?? process.env.DIKAROUTE_CONTEXT)?.baseUrl;
    } catch {
      /* none */
    }
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "DikaRoute",
      apiConfigs: {
        DikaRoute: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_dikaroute",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".dikaroute", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("DikaRoute → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { DikaRoute: { ...importDoc.providerProfiles.apiConfigs.DikaRoute, openAiApiKey: apiKey ? "set" : "sk_dikaroute" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your DIKAROUTE_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for DikaRoute: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.dikaroute/roo-settings.json)")
