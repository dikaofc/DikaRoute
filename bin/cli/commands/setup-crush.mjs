/**
 * dikaroute setup-crush — configure Crush (charmbracelet/crush) for DikaRoute.
 *
 ...
 * include /v1; the api_key may reference an env var (`$DIKAROUTE_API_KEY`) so the
...
const API_KEY_REF = "$DIKAROUTE_API_KEY";
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `DikaRoute: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the DikaRoute provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), dikaroute: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Crush (openai-compat)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${provider.models.length} model(s) under providers.dikaroute → ${configPath}`);
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.dikaroute)`);
  printInfo("Provide the key (config references $DIKAROUTE_API_KEY):  export DIKAROUTE_API_KEY=...");
...
export function registerSetupCrush(program) {
  program
    .command("setup-crush")
    .description("Generate the DikaRoute openai-compat provider in ~/.config/crush/crush.json")
