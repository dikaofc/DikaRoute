/** Configure Qwen Code's OpenAI-compatible provider for DikaRoute. */
...
export function resolveQwenTarget(opts = {}) {
  ...
  if (!root || !(opts.apiKey ?? opts["api-key"])) {
    try {
      context = resolveActiveContext(opts.context ?? process.env.DIKAROUTE_CONTEXT);
    } catch {
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Qwen Code (OpenAI-compatible)");
  ...
  try {
    ...
    if (dryRun) {
      ...
      printInfo(`[dry-run] credential → ${envPath} (DIKAROUTE_API_KEY)`);
      return 0;
    }
    ...
    printSuccess(`Updated ${envPath} (DIKAROUTE_API_KEY only)`);
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description("Configure Qwen Code's upstream V4 modelProviders format for DikaRoute")
    .option("--port <port>", "Local DikaRoute port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote DikaRoute URL")
    .option("--api-key <key>", "DikaRoute API key")
