/**
 * dikaroute setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) DikaRoute and
 ...
 *
 * Launch a profile with:  dikaroute launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `dikaroute
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Claude Code profile generator");
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  dikaroute launch --profile <name>     # e.g. dikaroute launch --profile glm52");
    console.log(
