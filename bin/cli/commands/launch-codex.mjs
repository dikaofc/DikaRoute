 ...
 *  in the shell can't shadow the dikaroute provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "dikaroute-no-auth";
 ...
 * (`model_providers.dikaroute.name="DikaRoute"` arrives unquoted and no longer
...
/**
 * Resolve the DikaRoute root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.DIKAROUTE_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.DIKAROUTE_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an DikaRoute root URL before launching Codex. */
 ...
 * DIKAROUTE_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.DIKAROUTE_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `dikaroute` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  DikaRoute root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "dikaroute"),
    "-c",
    tomlAssign("model_providers.dikaroute.name", "DikaRoute"),
    "-c",
    tomlAssign("model_providers.dikaroute.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.dikaroute.env_key", "DIKAROUTE_API_KEY"),
    "-c",
    tomlAssign("model_providers.dikaroute.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.dikaroute.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") ||
        "DikaRoute is not reachable at {port}. Start it with 'dikaroute serve'."
...
export function registerLaunchCodex(program) {
  program
    ...
    .description(
      t("launchCodex.description") || "Launch Codex CLI pointed at DikaRoute (local or remote VPS)"
    )
    .option("--port <port>", "Local DikaRoute port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote DikaRoute base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "DikaRoute API key (overrides DIKAROUTE_API_KEY env var for this invocation)"
    )
