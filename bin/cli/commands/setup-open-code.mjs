/**
 * dikaroute setup opencode — Wire the bundled @dikaroute/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g dikaroute` ships the plugin
 * inside the dikaroute package (`@dikaroute/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/dikaroute/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider dikaroute`
...
// We walk up from this file to find the dikaroute package root. The script
// lives at `<dikaroute>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@dikaroute/opencode-plugin/
// (see root package.json `files`: ["@dikaroute/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.DIKAROUTE_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@dikaroute", "opencode-plugin");
...
/**
 * Locate the bundled @dikaroute/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published dikaroute tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @dikaroute/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means dikaroute was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling dikaroute (npm install -g dikaroute) ` +
        `or run \`cd @dikaroute/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@dikaroute/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/dikaroute/`. We
 ...
 * resolve the peer deps from the dikaroute package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "dikaroute");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`dikaroute setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/dikaroute/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-dikaroute-auth` entry if present — that
  // package is the obsolete predecessor of @dikaroute/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-dikaroute-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-dikaroute-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * The bundled @dikaroute/opencode-plugin registers its provider under
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="dikaroute"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "dikaroute";
  // Remote-aware: explicit --remote/--base-url → active context → localhost.
  let baseURL = opts.remote || opts.baseURL || opts.baseUrl;
  if (!baseURL) {
    try {
      const ctx = resolveActiveContext(opts.context ?? process.env.DIKAROUTE_CONTEXT);
      baseURL = ctx?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  printHeading("DikaRoute → OpenCode Plugin Setup");
...
/**
 * Register the `dikaroute setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `dikaroute setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @dikaroute/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: dikaroute)",
      "dikaroute"
    )
    .option(
      "--base-url <url>",
      "DikaRoute base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote DikaRoute URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
