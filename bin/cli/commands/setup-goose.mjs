/**
 * dikaroute setup-goose — configure Goose (block/goose) for DikaRoute.
 *
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description("Configure Goose for DikaRoute: write ~/.config/goose/config.yaml + print the env recipe")
