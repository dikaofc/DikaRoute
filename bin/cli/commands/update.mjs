...
export async function getLatestVersion(execFn = execFileAsync) {
  try {
    const { stdout } = await execFn("npm", ["view", "dikaroute", "version", "--prefer-online"], {
      timeout: 15000,
    });
    return stdout.trim();
  } catch {
...
export async function createBackup() {
  ...
  const backupDir = path.join(homedir(), ".dikaroute", "backups", `dikaroute-${Date.now()}`);
  ...
  try {
    ...
    const files = ["dikaroute.mjs", "cli", "nodeRuntimeSupport.mjs", "mcp-server.mjs"];
...
export async function runUpdateCommand(opts = {}) {
  ...
  if (showChangelog) {
    try {
      const { stdout } = await execFileAsync("npm", ["view", "dikaroute", "changelog"], {
        timeout: 10000,
      });
      if (stdout.trim()) {
  ...
  if (checkOnly) {
    console.log("\n  Run `dikaroute update --apply` to install automatically.");
    return 1; // exit 1 = outdated (useful for scripts)
  }

  if (dryRun) {
    console.log("\n  [DRY RUN] Would run: npm install -g dikaroute@latest --include=optional");
    if (!skipBackup) console.log("  [DRY RUN] Would create backup in ~/.dikaroute/backups/");
    return 0;
  }
  ...
  try {
    ...
    execSync("npm install -g dikaroute@latest --include=optional", { stdio: "inherit" });
    // Trust-but-verify: `npm install -g` exits 0 even when a shadowing local install
    // (e.g. ~/node_modules/dikaroute ahead of the global prefix on PATH) means the
    ...
    if (afterVersion && compareVersions(afterVersion, latest) < 0) {
      ...
      console.log(
        "  A local `node_modules/dikaroute` is likely shadowing the global install on PATH."
      );
      console.log("  Diagnose with:");
      console.log("    which -a dikaroute");
      console.log("    command -v dikaroute");
      console.log("    npm prefix -g");
      console.log(
        "  Then remove the shadowing local copy (e.g. `npm uninstall dikaroute` from its directory)"
      );
      console.log("  or reorder PATH so the global bin comes first.");
      return 1;
    }
    printSuccess(`Updated to version ${latest}`);
    printInfo("Run `dikaroute --version` to verify.");
    return 0;
  } catch (err) {
    printError(`Update failed: ${err.message}`);
    printInfo("Restore from backup:");
    const backupDir = path.join(homedir(), ".dikaroute", "backups");
