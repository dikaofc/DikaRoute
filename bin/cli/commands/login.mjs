...
/**
 * `dikaroute login antigravity` — local OAuth helper for remote installs.
 *
 ...
 *
 * It talks ONLY to Google (no DikaRoute server needed locally), so it works even
 * if the remote VPS is firewalled from the user's machine.
 *
 * Push mode: when an active remote context exists (`dikaroute connect <host>`), the
...
function defaultStartServer(preferredPort) {
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>DikaRoute</title>" +
...
/** Read the active CLI context (baseUrl + scoped token) written by `dikaroute connect`. */
...
export async function runAntigravityLogin(opts = {}, deps = {}) {
  ...
  print(
    "\n" +
      ...
      'DikaRoute dashboard: Providers → Antigravity → Connect → "Paste credentials".\n' +
...
export function registerLogin(program) {
  const login = program
    .command("login")
    .description("Local OAuth helpers for remote DikaRoute installs (run on your own machine)");
