/**
 * dikaroute setup-cursor — guide Cursor to use DikaRoute.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your DikaRoute key (DIKAROUTE_API_KEY)",
...
export function registerSetupCursor(program) {
  program
    ...
    .description("Print the steps to point Cursor at DikaRoute (chat panel; Cursor config is not file-writable)")
