/**
 * dikaroute setup-cline — configure the Cline AI coding agent to use DikaRoute.
 *
 ...
 * writes the CLI-mode files (matching the DikaRoute dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.DIKAROUTE_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge DikaRoute openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_dikaroute" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_dikaroute"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for DikaRoute: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
