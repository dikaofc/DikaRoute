...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or DIKAROUTE_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `dikaroute setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("DikaRoute Setup");
    const { db, dbPath } = await openDikaRouteDb();
