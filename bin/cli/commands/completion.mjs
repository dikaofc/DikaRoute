...
function readCache() {
  try {
    ...
    if (process.env.DIKAROUTE_DEBUG_COMPLETION) {
      console.error("[dikaroute completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.DIKAROUTE_DEBUG_COMPLETION) {
      console.error("[dikaroute completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.DIKAROUTE_DEBUG_COMPLETION) {
      console.error("[dikaroute completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_dikaroute");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "dikaroute.fish");
  return join(home, ".bash_completion.d", "dikaroute");
}

function generateZshScript() {
  return `#compdef dikaroute

# DikaRoute zsh completion (dynamic)
_dikaroute_get_cache() {
  local key="$1"
  local cache="$HOME/.dikaroute/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    dikaroute completion refresh --quiet >/dev/null 2>&1
  fi
...
_dikaroute() {
  local -a commands
  commands=(
    'serve:Start the DikaRoute server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure DikaRoute'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_dikaroute_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_dikaroute_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_dikaroute_get_cache models))
...
compdef _dikaroute dikaroute
`;
}

function generateBashScript() {
  return `#!/bin/bash
# DikaRoute CLI bash completion (dynamic)
...
complete -F _dikaroute dikaroute
`;
}

function generateFishScript() {
  return `# DikaRoute CLI fish completion (dynamic)
complete -c dikaroute -f
...
for cmd in $commands
  complete -c dikaroute -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c dikaroute -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c dikaroute -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c dikaroute -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c dikaroute -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c dikaroute -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c dikaroute -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __dikaroute_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.dikaroute/completion-cache.json"
...
complete -c dikaroute -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__dikaroute_cache_get combos)'
complete -c dikaroute -l model -a '(__dikaroute_cache_get models)'
complete -c dikaroute -l combo -a '(__dikaroute_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `dikaroute completion <shell>` (positional arg form)
  comp
    .command("<shell>")
