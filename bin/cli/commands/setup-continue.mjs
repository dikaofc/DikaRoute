/**
 * dikaroute setup-continue — configure Continue (continue.dev) for DikaRoute.
 *
 ...
 * `apiKey: ${{ secrets.DIKAROUTE_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.DIKAROUTE_API_KEY }}";
...
/**
 * Merge DikaRoute models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "DikaRoute Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("DikaRoute → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} DikaRoute model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} DikaRoute models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export DIKAROUTE_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'DIKAROUTE_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the DikaRoute model catalog"
    )
