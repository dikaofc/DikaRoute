 ...
 *
 * Purely informational invocations must NOT create `~/.dikaroute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `dikaroute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `dikaroute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `dikaroute` runs the default `serve` command → must provision.
