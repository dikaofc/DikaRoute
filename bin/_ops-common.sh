# bin/_ops-common.sh — shared helpers for the DikaRoute ops runbook scripts.
#
...
ops_set_data_dir() {
  DIKAROUTE_DATA_DIR="$1"
  DIKAROUTE_SQLITE="${DIKAROUTE_DATA_DIR}/storage.sqlite"
  DIKAROUTE_BACKUPS_DIR="${DB_BACKUPS_DIR:-${DIKAROUTE_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.dikaroute}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$DIKAROUTE_BACKUPS_DIR/$id" \
    "$DIKAROUTE_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$DIKAROUTE_BACKUPS_DIR" ]; then
    for cand in "$DIKAROUTE_BACKUPS_DIR"/snapshot_*"$id"*; do
