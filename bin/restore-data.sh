#!/usr/bin/env bash
# bin/restore-data.sh — restore the DikaRoute SQLite data volume from a snapshot
...
Stop DikaRoute before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $DIKAROUTE_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $DIKAROUTE_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$DIKAROUTE_SQLITE" ]; then
  safety="$DIKAROUTE_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$DIKAROUTE_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$DIKAROUTE_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$DIKAROUTE_SQLITE" "$safety/storage.sqlite"
  fi
