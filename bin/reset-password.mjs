 ...
 *   dikaroute reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | dikaroute reset-password
 *   printf 'NewPass123' | dikaroute reset-password --password-stdin
 *
 * Resets the admin password for DikaRoute.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure DikaRoute has been started at least once.`);
  ...
  console.log("   Restart DikaRoute for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/dikaroute.mjs
    // routes `dikaroute reset-password` here) terminates cleanly instead of
