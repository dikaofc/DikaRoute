const CACHE_NAME = "dikaroute-pwa-v2";
const APP_SHELL = [
  "/",
  "/offline",
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "DikaRoute", body: event.data?.text() || "New notification" };
  }

  const title = data.title || "DikaRoute";
  const options = {
    body: data.body || "",
    icon: data.icon || "/icon-512.png",
    badge: data.badge || "/icon-192.png",
    tag: data.tag || "dikaroute-default",
    data: {
