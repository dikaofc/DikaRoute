...
# escape hatch: `--build-arg`/-e DIKAROUTE_USE_TURBOPACK=0.
# See docs/ops/QUALITY_GATE_PLAYBOOK.md Parte 6.
ENV DIKAROUTE_USE_TURBOPACK=1

# Next.js basePath is fixed at build time; pass DIKAROUTE_BASE_PATH here when the
# image should serve under a reverse-proxy subpath without a runtime patch.
ARG DIKAROUTE_BASE_PATH=""
ENV DIKAROUTE_BASE_PATH=$DIKAROUTE_BASE_PATH
...
# (DIKAROUTE_MEMORY_MB). Override: `--build-arg DIKAROUTE_BUILD_MEMORY_MB=6144`.
ARG DIKAROUTE_BUILD_MEMORY_MB=4096
ENV NODE_OPTIONS="--max-old-space-size=${DIKAROUTE_BUILD_MEMORY_MB}"
...
LABEL org.opencontainers.image.title="dikaroute" \
  org.opencontainers.image.description="Unified AI proxy — route any LLM through one endpoint" \
  org.opencontainers.image.url="https://dikaroute.online" \
  org.opencontainers.image.source="https://github.com/ObitoGlory" \
...
# `-e DIKAROUTE_MEMORY_MB=2048` (or higher) if you raise fusionTuning.maxPanel
# above the default cap.
ENV DIKAROUTE_MEMORY_MB=1024
ENV NODE_OPTIONS="--max-old-space-size=${DIKAROUTE_MEMORY_MB}"
...
ENV DIKAROUTE_MIGRATIONS_DIR=/app/migrations
...
#
#  Two image flavors:
#    runner-base  →  dikaroute:VERSION        Lean base (~500 MB). No browsers.
#    runner-web   →  dikaroute:VERSION-web    +Chromium/Playwright (~800 MB).
#
...
#
#  Build:
#    docker build --target runner-web -t dikaroute:web .
#  Compose:
#    build:
#      context: .
#      target: runner-web
