/**
 * OpenCode provider plugin for DikaRoute AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * DikaRoute instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any DikaRoute-exposed
 ...
 *       apiKey: "sk_dikaroute",
 *     });
 *     // config -> { $schema, provider: { dikaroute: { npm, name, options, models } } }
 *     ```
 *
 ...
 *     const provider = createDikaRouteProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.dikaroute in opencode.json
 *     ```
 *
...
export const DIKAROUTE_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed DikaRoute models. Synced
 * with the Alph4d0g/opencode-dikaroute-auth DIKAROUTE_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-dikaroute-auth, MIT) and extended
 * with Claude Code passthrough models (`cc/` prefix).
 */
export const DIKAROUTE_DEFAULT_OPENCODE_MODELS = [
  "cc/claude-opus-4-8",
  "cc/claude-opus-4-7",
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-dikaroute-auth
 * (https://github.com/Alph4d0g/opencode-dikaroute-auth, MIT).
 */
 ...
 * Matches the context lengths used by DikaRoute's provider registry.
 */
export const DIKAROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `DikaRouteProviderOptions.modelCapabilities`.
 */
export const DIKAROUTE_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface DikaRouteProviderOptions {
  /** DikaRoute base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** DikaRoute API key. Required. Use `sk_dikaroute` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"DikaRoute"`. */
  displayName?: string;
   ...
   * `DIKAROUTE_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `DIKAROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"dikaroute/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.dikaroute.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to DikaRoute. */
  npm: typeof DIKAROUTE_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"dikaroute/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"dikaroute/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [DIKAROUTE_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@dikaroute/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@dikaroute/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@dikaroute/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.dikaroute` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createDikaRouteProvider(options: DikaRouteProviderOptions): OpenCodeProviderEntry {
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = DIKAROUTE_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      DIKAROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: DIKAROUTE_PROVIDER_NPM,
    name: options.displayName?.trim() || "DikaRoute",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.dikaroute`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"dikaroute/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildDikaRouteOpenCodeConfig(
  options: DikaRouteProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [DIKAROUTE_PROVIDER_KEY]: createDikaRouteProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${DIKAROUTE_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${DIKAROUTE_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the DikaRoute provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.dikaroute` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildDikaRouteOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [DIKAROUTE_PROVIDER_KEY]: partial.provider[DIKAROUTE_PROVIDER_KEY],
  };
 ...
 */
export const DIKAROUTE_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type DikaRouteMCPScope = (typeof DIKAROUTE_MCP_DEFAULT_SCOPES)[number] | string;

export interface DikaRouteMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** DikaRoute API key forwarded to the MCP server as `DIKAROUTE_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `DIKAROUTE_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `DIKAROUTE_MCP_SCOPES`.
   * When omitted `DIKAROUTE_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: DikaRouteMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.dikaroute` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createDikaRouteMCPEntry({
 *   serverPath: "/home/user/.local/share/dikaroute/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_dikaroute",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.dikaroute
 * ```
 */
export function createDikaRouteMCPEntry(options: DikaRouteMCPOptions): OpenCodeMCPServerEntry {
  ...
  const env: Record<string, string> = {
    DIKAROUTE_API_KEY: apiKey,
  };

  if (options.managementApiKey !== undefined) {
    const mgmtKey = options.managementApiKey.trim();
    if (mgmtKey) env.DIKAROUTE_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.DIKAROUTE_MCP_ENFORCE_SCOPES = "true";
    env.DIKAROUTE_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@dikaroute/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that DikaRoute's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different DikaRoute releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-dikaroute-auth (MIT).
 */
export interface DikaRouteLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running DikaRoute instance.
 *
 ...
 * `display_name`) field variants across DikaRoute versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createDikaRouteProvider` / `buildDikaRouteOpenCodeConfig` instead of
 * relying on `DIKAROUTE_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - DikaRoute base URL (with or without `/v1`).
 * @param apiKey    - DikaRoute API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_dikaroute");
 * const config = buildDikaRouteOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_dikaroute",
 *   models,                    // DikaRouteLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  const models: DikaRouteLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from DikaRoute's /v1/models response.
    // DikaRoute returns context_length in snake_case for both synced
 ...
 */
export type DikaRouteCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface DikaRouteCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: DikaRouteCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running DikaRoute instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as DikaRouteCompressionOverride)
      : "";
...
/**
 * Options for `createDikaRouteComboConfig`.
 * Mirrors the subset of combo fields exposed by the DikaRoute `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface DikaRouteComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for DikaRoute's management API.
 *
 ...
 * ```
 */
export function createDikaRouteComboConfig(
  options: DikaRouteComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface DikaRouteRoleOverrides {
...
/** Per-role binding used by `createDikaRouteAgentBlock`. */
export interface DikaRouteAgentRole extends DikaRouteRoleOverrides {
  /** DikaRoute model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createDikaRouteAgentBlock`. */
export interface DikaRouteAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, DikaRouteAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends DikaRouteRoleOverrides {
  /** Always emitted as `"dikaroute/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: DikaRouteAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${DIKAROUTE_PROVIDER_KEY}/${modelId}` };
 ...
 * specific DikaRoute model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createDikaRouteAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "dikaroute/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createDikaRouteAgentBlock(
  options: DikaRouteAgentBlockOptions
...
/**
 * Per-mode binding used by `createDikaRouteModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `DikaRouteAgentRole` + `createDikaRouteAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface DikaRouteMode extends DikaRouteAgentRole {}

/**
 * Options for `createDikaRouteModesBlock`.
 *
 * @deprecated See `DikaRouteMode`.
 */
export interface DikaRouteModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, DikaRouteMode>;
}
 ...
 * a specific DikaRoute model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createDikaRouteAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createDikaRouteModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createDikaRouteModesBlock(
  options: DikaRouteModesBlockOptions
