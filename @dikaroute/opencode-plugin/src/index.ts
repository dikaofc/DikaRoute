/**
 * OpenCode plugin for the DikaRoute AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running DikaRoute instance from
 ...
 *                  pass-through (DikaRoute is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "dikaroute"`):
 *
 *     ```json
 *     {
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@dikaroute/opencode-plugin", { "providerId": "dikaroute" }],
 *         ["@dikaroute/opencode-plugin", { "providerId": "dikaroute-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@dikaroute/opencode-provider` (build-time config generator)
 ...
 * @see https://github.com/ObitoGlory for the AI Gateway.
 */
 ...
 *                     different `providerId` ("dikaroute", "dikaroute-preprod",
 *                     "dikaroute-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this DikaRoute instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/dikaroute-debug-{providerId}.jsonl`.
 ...
 *                           (covers DikaRoute's canonical Anthropic aliases).
 *
 ...
 * import `DikaRoutePluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type DikaRoutePluginOptions = z.infer<typeof optionsSchema>;
 ...
 */
export const DIKAROUTE_FEATURE_DEFAULTS = {
...
/** Union of the boolean feature-flag keys declared in `DIKAROUTE_FEATURE_DEFAULTS`. */
export type DikaRouteFeatureFlag = keyof typeof DIKAROUTE_FEATURE_DEFAULTS;
...
export function resolveEffectiveFeatureFlags(
  features?: DikaRoutePluginOptions["features"]
): Record<DikaRouteFeatureFlag, boolean> {
  const f: Partial<Record<DikaRouteFeatureFlag, boolean>> = features ?? {};
  const out = {} as Record<DikaRouteFeatureFlag, boolean>;
  for (const key of Object.keys(DIKAROUTE_FEATURE_DEFAULTS) as DikaRouteFeatureFlag[]) {
    const val = f[key];
    out[key] = typeof val === "boolean" ? val : DIKAROUTE_FEATURE_DEFAULTS[key];
  }
  return out;
}

export const DIKAROUTE_PROVIDER_KEY = "dikaroute" as const;
 ...
 */
export function resolveDikaRoutePluginOptions(opts?: DikaRoutePluginOptions): Required<
  Pick<
    DikaRoutePluginOptions,
    "providerId" | "displayName" | "modelCacheTtl" | "autoSyncIntervalMs"
  >
> & {
  /**
   * #6859: the UNPREFIXED provider id ("dikaroute", "dikaroute-preprod", …).
   ...
   * `provider.<id>` block). `dikarouteProviderId` MUST be used everywhere an
   * identifier reaches or represents something DikaRoute's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — DikaRoute's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  dikarouteProviderId: string;
} & Pick<DikaRoutePluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? DIKAROUTE_PROVIDER_KEY;
  const dikarouteProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "dikaroute") keep working.
  const providerId = rawProviderId.startsWith("opencode-")
    ? rawProviderId
    : `opencode-${rawProviderId}`;
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${DIKAROUTE_PROVIDER_KEY}`
      ? "DikaRoute"
      : `DikaRoute (${providerId})`);
  const modelCacheTtl =
...
export type ResolvedDikaRoutePluginOptions = ReturnType<typeof resolveDikaRoutePluginOptions>;

/**
 * Strip a leading "opencode-" prefix (added only for the OC native-adapter
 * gate — see `resolveDikaRoutePluginOptions`) so the returned id is safe to
 * embed in anything DikaRoute's own server parses. A user-supplied
 * `providerId: "opencode-dikaroute"` (already prefixed) resolves to the same
 * unprefixed "dikaroute" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `DikaRoutePluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `DikaRoutePlugin`),
 ...
 */
export function parseDikaRoutePluginOptions(opts: unknown): DikaRoutePluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @dikaroute/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseDikaRoutePluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): DikaRoutePluginOptions {
  return parseDikaRoutePluginOptions(opts);
}
 ...
 * Covers DikaRoute's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-dikaroute-auth@1.2.1 which pinned
 *     `DIKAROUTE_PROVIDER_ID = "dikaroute"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; DikaRoute issues bearer
 ...
 */
export function createDikaRouteAuthHook(opts?: DikaRoutePluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveDikaRoutePluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `DikaRoute API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createDikaRouteFetchInterceptor({
            apiKey,
 ...
 */
export function invalidateDikaRouteFetchCache(
  cache: DikaRouteFetchCache,
  baseURL?: string
): number {
  if (!baseURL) {
    const n = cache.size;
 ...
 */
export async function resolveDikaRouteRuntimeAuth(
  resolved: ResolvedDikaRoutePluginOptions,
  readAuthJson?: DikaRouteReadAuthJson
  ...
  if (resolved.dikarouteProviderId && !lookupKeys.includes(resolved.dikarouteProviderId)) {
    lookupKeys.push(resolved.dikarouteProviderId);
  }
...
/**
 * Force-refresh DikaRoute catalog: clear memory + disk cache, re-fetch /v1/models
 * (and optional management endpoints), and repopulate the shared cache.
 * OpenCode equivalent of Pi `/omni sync`.
 */
export async function forceSyncDikaRouteModels(args: {
  resolved: ResolvedDikaRoutePluginOptions;
  cache: DikaRouteFetchCache;
  readAuthJson?: DikaRouteReadAuthJson;
  fetcher?: DikaRouteModelsFetcher;
  combosFetcher?: DikaRouteCombosFetcher;
  autoCombosFetcher?: DikaRouteAutoCombosFetcher;
  enrichmentFetcher?: DikaRouteEnrichmentFetcher;
  compressionMetaFetcher?: DikaRouteCompressionMetaFetcher;
  providersFetcher?: DikaRouteProvidersFetcher;
  now?: () => number;
}): Promise<{
  ok: boolean;
  count: number;
  ...
  const fetcher = args.fetcher ?? defaultDikaRouteModelsFetcher;
  const combosFetcher = args.combosFetcher ?? defaultDikaRouteCombosFetcher;
  const autoCombosFetcher = args.autoCombosFetcher ?? defaultDikaRouteAutoCombosFetcher;
  const enrichmentFetcher = args.enrichmentFetcher ?? defaultDikaRouteEnrichmentFetcher;
  const compressionMetaFetcher =
    args.compressionMetaFetcher ?? defaultDikaRouteCompressionMetaFetcher;
  const providersFetcher = args.providersFetcher ?? defaultDikaRouteProvidersFetcher;
  ...
  const auth = await resolveDikaRouteRuntimeAuth(
    resolved,
  ...
  if (!auth) {
    return {
      ...
      count: 0,
      combos: 0,
      provider: resolved.dikarouteProviderId,
      clearedMemory: 0,
      clearedDisk: false,
      error:
        "No DikaRoute credentials/baseURL available. Run `opencode connect dikaroute` or set plugin baseURL.",
    };
  }

  const clearedMemory = invalidateDikaRouteFetchCache(cache, auth.baseURL);
  // Clear residual entries from prior baseURL history as well.
  const clearedAll = invalidateDikaRouteFetchCache(cache);
  let clearedDisk = false;
  if (wantDiskCache) {
    clearedDisk = await clearDiskSnapshot(resolved.providerId);
    if (resolved.dikarouteProviderId !== resolved.providerId) {
      clearedDisk = (await clearDiskSnapshot(resolved.dikarouteProviderId)) || clearedDisk;
    }
  }

  try {
    const rawModels = await fetcher(auth.baseURL, auth.apiKey, 10_000);
    let rawCombos: DikaRouteRawCombo[] = [];
    if (wantCombos) {
      try {
        rawCombos = await combosFetcher(auth.baseURL, auth.managementReadToken, 10_000);
      } catch (err) {
        console.warn("[dikaroute-plugin] force sync: combos fetch failed", err);
      }
    }
    let rawAutoCombos: DikaRouteRawAutoCombo[] = [];
    if (wantAutoCombos) {
      try {
    ...
    let rawEnrichment: DikaRouteEnrichmentMap = new Map();
    if (wantEnrichment) {
      try {
    ...
    let rawCompressionCombos: DikaRouteCompressionCombo[] = [];
    ...
    let rawConnections: DikaRouteProviderConnection[] = [];
    if (wantUsableOnly) {
      try {
    ...
    console.warn(
      `[dikaroute-plugin] force sync ok providerId=${resolved.providerId} ` +
...
export function createDikaRouteSyncModelsTool(args: {
  resolved: ResolvedDikaRoutePluginOptions;
  cache: DikaRouteFetchCache;
  ...
  return tool({
    description:
      "Force-refresh the DikaRoute model catalog (OpenCode equivalent of Pi `/omni sync`). " +
    ...
    async execute(toolArgs) {
      const result = await forceSyncDikaRouteModels({ resolved, cache });
      const reason = toolArgs.reason ? ` reason=${toolArgs.reason}` : "";
      if (!result.ok) {
        return {
          title: "DikaRoute sync failed",
          output:
            `DikaRoute model sync failed for provider=${result.provider}.${reason}\n` +
            `${result.error ?? "unknown error"}`,
          metadata: result,
        };
      }
      return {
        title: `DikaRoute sync: ${result.count} models`,
        output:
          `DikaRoute models synced.` +
 ...
 */
export function startDikaRouteAutoSync(args: {
  resolved: ResolvedDikaRoutePluginOptions;
  cache: DikaRouteFetchCache;
  intervalMs?: number;
}): () => void {
  ...
  const tick = () => {
    ...
    inFlight = (async () => {
      ...
      if (!result.ok) {
        console.warn(
          `[dikaroute-plugin] auto-sync failed providerId=${resolved.providerId}: ${result.error}`
        );
        return;
      }
      ...
      if (result.count !== lastCount) {
        console.warn(
          `[dikaroute-plugin] auto-sync catalog size changed ${lastCount} → ${result.count} ` +
      ...
      .catch((err) => {
        console.warn("[dikaroute-plugin] auto-sync tick error", err);
      })
      .finally(() => {
        inFlight = null;
      });
  ...
  console.warn(
    `[dikaroute-plugin] auto-sync enabled intervalMs=${intervalMs} providerId=${resolved.providerId}`
  );
...
export const DikaRoutePlugin: Plugin = async (_input, options) => {
  const resolved = resolveDikaRoutePluginOptions(coercePluginOptions(options));
  ...
  // Each `DikaRoutePlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: DikaRouteFetchCache = new Map();
  ...
  startDikaRouteAutoSync({ resolved, cache: sharedCache });

  const syncTool = createDikaRouteSyncModelsTool({ resolved, cache: sharedCache });
  const bareProviderId = resolved.dikarouteProviderId;
  ...
  const baseConfigHook = createDikaRouteConfigHook(resolved, { cache: sharedCache });
  const configWithSyncCommand = async (input: Config) => {
    ...
    if (!cfg.command["omni-sync"]) {
      cfg.command["omni-sync"] = {
        description: "Force-refresh DikaRoute model catalog (like Pi /omni sync)",
        template:
          `Force-refresh the DikaRoute model catalog now using the dikaroute_sync_models tool ` +
    ...
    if (!cfg.command["omni-autosync"]) {
      cfg.command["omni-autosync"] = {
        description: "Show DikaRoute auto-sync / cache status",
        template:
          `Report DikaRoute discovery status for provider ${bareProviderId}: ` +
          `autoSyncIntervalMs=${resolved.autoSyncIntervalMs}, modelCacheTtl=${resolved.modelCacheTtl}. ` +
          `If the user asked to refresh now, call dikaroute_sync_models.`,
      };
    }
  };

  return {
    auth: createDikaRouteAuthHook(resolved),
    provider: createDikaRouteProviderHook(resolved, { cache: sharedCache }),
    config: configWithSyncCommand,
    tool: {
      dikaroute_sync_models: syncTool,
    },
  };
};
 ...
 * constants (DIKAROUTE_PROVIDER_KEY, DEFAULT_MODEL_CACHE_TTL_MS) + types +
 ...
 */
const DikaRouteV1Plugin = {
  id: "@dikaroute/opencode-plugin",
  server: DikaRoutePlugin,
};

export default DikaRouteV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from DikaRoute. Captured verbatim from
 ...
 */
export interface DikaRouteRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * DikaRoute instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@dikaroute/opencode-provider`'s
 ...
 */
export type DikaRouteModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope DikaRoute
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future DikaRoute build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultDikaRouteModelsFetcher: DikaRouteModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@dikaroute/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("@dikaroute/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `@dikaroute/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
      );
    }
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: DikaRouteRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DikaRouteRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from DikaRoute has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from DikaRoute has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per DikaRoute convention (OpenAI-compat mode
 ...
 *      Unknown strings (future DikaRoute additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — DikaRoute doesn't tier models alpha/beta on
...
/**
 * Raw shape of a single combo entry as returned by DikaRoute's `/api/combos`.
 *
 * Schema established via a live probe against
 * an DikaRoute `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future DikaRoute
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@dikaroute/opencode-provider#listCombos`).
 *
 ...
 */
export interface DikaRouteRawComboMemberRef {
...
export interface DikaRouteRawCombo {
  id: string;
  name?: string;
  ...
  models?: DikaRouteRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When DikaRoute attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `DikaRouteModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type DikaRouteCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteRawCombo[]>;
 ...
 * plugin working if a future DikaRoute build trims the wrapper).
 *
 * Differences from `defaultDikaRouteModelsFetcher`:
 ...
 */
export const defaultDikaRouteCombosFetcher: DikaRouteCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@dikaroute/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL)
    throw new Error("@dikaroute/opencode-plugin: baseURL required to fetch /api/combos");
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: DikaRouteRawCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DikaRouteRawCombo);
      }
    }
    return out;
  } finally {
...
export function mapComboToModelV2(
  combo: DikaRouteRawCombo,
  members: DikaRouteRawModelEntry[],
  providerId: string,
  baseURL: string,
  apiFormat?: { anthropicPrefixes?: string[] }
): ModelV2 {
...
/**
 * Raw shape of an auto combo entry as returned by DikaRoute's
 ...
 */
export interface DikaRouteRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer DikaRoute builds.
 ...
 */
export type DikaRouteAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteRawAutoCombo[]>;
 ...
 */
export const defaultDikaRouteAutoCombosFetcher: DikaRouteAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000
) => {
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(
        `[dikaroute-plugin] /api/combos/auto not available (404) — auto combos disabled`
      );
      return [];
    }
    if (!res.ok) {
      console.warn(
        `[dikaroute-plugin] /api/combos/auto failed: ${res.status} ${res.statusText} — auto combos disabled`
      );
      return [];
    }
    const body = (await res.json()) as unknown;
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: DikaRouteRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as DikaRouteRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
    ...
    console.warn(
      `[dikaroute-plugin] /api/combos/auto fetch failed: ${err instanceof Error ? err.message : String(err)} — auto combos disabled`
    );
    return [];
  } finally {
    clearTimeout(timer);
 ...
 * DikaRoute builds). MUST be positive: OpenCode's overflow guard treats
...
export function mapAutoComboToStaticEntry(
  autoCombo: DikaRouteRawAutoCombo
): DikaRouteStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from DikaRoute's
 ...
 */
export interface DikaRouteEnrichmentEntry {
   ...
   * Populated by `defaultDikaRouteEnrichmentFetcher` from
...
export type DikaRouteEnrichmentMap = Map<string, DikaRouteEnrichmentEntry>;

export type DikaRouteEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteEnrichmentMap>;
 ...
 * other.
 */
export const defaultDikaRouteEnrichmentFetcher: DikaRouteEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: DikaRouteEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // DikaRoute hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: DikaRouteEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<DikaRouteEnrichmentEntry["pricing"]> = {};
            // DikaRoute `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  enrichment: DikaRouteEnrichmentMap;
  autoCombos: DikaRouteRawAutoCombo[];
  features?: DikaRoutePluginOptions["features"];
}): Promise<void> {
  const {
    providerId,
  ...
  lines.push(
    `features(effective): ` +
      (Object.keys(effectiveFlags) as DikaRouteFeatureFlag[])
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "dikaroute-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving DikaRouteEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: DikaRouteEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * DikaRoute's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: DikaRouteEnrichmentMap | undefined
): Map<string, string> {
...
): DikaRouteEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<DikaRouteRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: DikaRoute's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `DikaRouteEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: DikaRouteEnrichmentMap | undefined
): Map<string, DikaRouteEnrichmentEntry> {
  const out = new Map<string, DikaRouteEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: DikaRouteEnrichmentEntry | undefined,
  aliasIndex: Map<string, DikaRouteEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): DikaRouteEnrichmentEntry | undefined {
  if (direct) {
...
export interface DikaRouteCompressionStep {
...
export interface DikaRouteCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: DikaRouteCompressionStep[];
  isDefault?: boolean;
}

export type DikaRouteCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteCompressionCombo[]>;
 ...
 */
export const defaultDikaRouteCompressionMetaFetcher: DikaRouteCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DikaRouteCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: DikaRouteCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: DikaRouteCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: DikaRouteCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: DikaRouteCompressionCombo = { id, pipeline: steps };
 ...
 * DikaRoute knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: DikaRouteCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed DikaRoute
...
export interface DikaRouteProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type DikaRouteProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DikaRouteProviderConnection[]>;
 ...
 */
export const defaultDikaRouteProvidersFetcher: DikaRouteProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DikaRouteProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: DikaRouteProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the DikaRoute deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: DikaRouteProviderConnection[],
  enrichment: DikaRouteEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultDikaRouteEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by DikaRoute's
 ...
 * (e.g. `dikaroute/MASTER`, `dikaroute/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`dikaroute/…`), never a
 ...
 * "Unable to determine provider", whereas `dikaroute/MASTER` resolves provider=
 * `dikaroute` and the openai-compatible adapter strips the prefix and sends the
 ...
 * OC user may register prod + preprod DikaRoute side-by-side with distinct
 ...
 *     DikaRoutePlugin instance — sharing the cache means /v1/models +
 *     /api/combos each hit the gateway exactly ONCE per TTL refresh, not
 *     twice.
 */
export interface DikaRouteFetchCacheEntry {
  rawModels: DikaRouteRawModelEntry[];
  rawCombos: DikaRouteRawCombo[];
  rawAutoCombos: DikaRouteRawAutoCombo[];
  /** Display-name + pricing overlay from /api/pricing/models. Empty Map when feature is disabled or fetch failed. */
  rawEnrichment: DikaRouteEnrichmentMap;
  /** Compression combos from /api/context/combos. Empty array when feature is disabled or fetch failed. */
  rawCompressionCombos: DikaRouteCompressionCombo[];
  /** Provider connections from /api/providers. Empty array when feature is disabled or fetch failed. */
  rawConnections: DikaRouteProviderConnection[];
  expiresAt: number;
}

export type DikaRouteFetchCache = Map<string, DikaRouteFetchCacheEntry>;
 ...
 *   - **Combo / model ID collisions: combos win.** DikaRoute treats combos
 ...
 *     `/api/combos` requires a management-scoped key and DikaRoute may
 ...
 *             outside tests today).
 */
export function createDikaRouteProviderHook(
  opts?: DikaRoutePluginOptions,
  deps: {
    fetcher?: DikaRouteModelsFetcher;
    combosFetcher?: DikaRouteCombosFetcher;
    autoCombosFetcher?: DikaRouteAutoCombosFetcher;
  ...
  const resolved = resolveDikaRoutePluginOptions(opts);
  const fetcher = deps.fetcher ?? defaultDikaRouteModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultDikaRouteCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultDikaRouteAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultDikaRouteEnrichmentFetcher;
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultDikaRouteCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultDikaRouteProvidersFetcher;
  ...
  const cache: DikaRouteFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      if (!baseURL) {
        console.warn(
          `[dikaroute-plugin] provider.models(${resolved.providerId}): ` +
      ...
      let rawModels: DikaRouteRawModelEntry[];
      let rawCombos: DikaRouteRawCombo[];
      let rawAutoCombos: DikaRouteRawAutoCombo[];
      let rawEnrichment: DikaRouteEnrichmentMap;
      let rawCompressionCombos: DikaRouteCompressionCombo[];
      let rawConnections: DikaRouteProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and DikaRoute may not have any combos
        ...
        if (wantCombos) {
          try {
            ...
            console.warn(
              "[dikaroute-plugin] combos fetch failed, falling back to models-only catalog",
              err
            );
          }
        }
        ...
        // may not exist yet on older DikaRoute versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn(
              "[dikaroute-plugin] enrichment fetch failed, falling back to raw ids",
              err
            );
          }
        }
        ...
        if (wantCompressionMeta) {
          try {
            ...
            console.warn("[dikaroute-plugin] compression-metadata fetch failed", err);
          }
        }
        ...
        if (wantUsableOnly) {
          try {
            ...
            console.warn(
              "[dikaroute-plugin] /api/providers fetch failed; usableOnly filter disabled for this refresh",
              err
            );
          }
        }
        ...
        // the dynamic pipeline fired and how much catalog DikaRoute returned.
        // Emitted once per cache miss (TTL refresh) — quiet on cache hits.
        console.warn(
          `[dikaroute-plugin] catalog refreshed for providerId=${resolved.providerId} baseURL=${baseURL}: ` +
      ...
      const rawModelById = new Map<string, DikaRouteRawModelEntry>();
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.dikarouteProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `dikaroute/claude-primary`), so we
      ...
      // compression metadata is present). DikaRoute returns at most one
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const memberEntries: DikaRouteRawModelEntry[] = [];
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as DikaRouteRawModelEntry);
              continue;
            }
          ...
          // Apply enrichment overlay to combos too (DikaRoute's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.dikarouteProviderId);
          ...
          if (Object.prototype.hasOwnProperty.call(models, comboKey)) {
            ...
            if (!isIntentionalDedup) {
              ...
              if (!collisionWarned.has(dedupeKey)) {
                ...
                console.warn(
                  `[dikaroute-plugin] combo key "${comboKey}" collides with a model id; combo wins.`
                );
              }
            }
          }
      ...
      if (pending.length > 0) {
        console.warn(
          `[dikaroute-plugin] ${pending.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; they will advertise context=0 to avoid over-claiming.`
        );
      }
...
// same-origin DikaRoute inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured DikaRoute origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-dikaroute-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same DikaRoute provider.
 *
 ...
 */
export function createDikaRouteFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
 ...
 * validator throws on them, breaking DikaRoute → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-dikaroute-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createDikaRouteFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[dikaroute-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@dikaroute/opencode-provider/src/index.ts` (`createDikaRouteProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@dikaroute/opencode-provider`. Stripped
 ...
 * when DikaRoute didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across DikaRoute deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type DikaRouteModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<DikaRouteModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): DikaRouteModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: DikaRouteModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as DikaRouteModalityKind;
...
export interface DikaRouteStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `dikaroute/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from DikaRoute `/api/pricing` shape:
   ...
   * DikaRoute's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: DikaRouteModalityKind[];
    output: DikaRouteModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@dikaroute/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — DikaRoute exposes an
 ...
 */
export interface DikaRouteStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, DikaRouteStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createDikaRouteProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: DikaRouteRawModelEntry[],
  rawCombos: DikaRouteRawCombo[],
  opts: ReturnType<typeof resolveDikaRoutePluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: DikaRouteEnrichmentMap,
  compressionCombos?: DikaRouteCompressionCombo[],
  connections?: DikaRouteProviderConnection[],
  rawAutoCombos?: DikaRouteRawAutoCombo[]
): DikaRouteStaticProviderEntry {
  const models: Record<string, DikaRouteStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. DikaRoute
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `dikaroute/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: DikaRouteStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when DikaRoute surfaced any. Without this field
    ...
    // DikaRoute field names to OC's static-schema field names.
    const pricing = enrichmentEntry?.pricing;
    if (pricing && (typeof pricing.input === "number" || typeof pricing.output === "number")) {
      const cost: NonNullable<DikaRouteStaticModelEntry["cost"]> = {
  ...
  const resolvedStaticCombosByName = new Map<string, DikaRouteStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`dikaroute/<slug>`) is what OC parses.
      const entry: DikaRouteStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<DikaRouteModalityKind>[]): DikaRouteModalityKind[] => {
      ...
      // `opts.providerId` reaches DikaRoute's server doubled
      // (`opencode-dikaroute/opencode-dikaroute/<slug>`), and `parseModel()`
      // resolves credentials for the nonexistent provider `opencode-dikaroute`
      // instead of `dikaroute`. See #7976.
      models[buildComboKey(combo, usedComboKeys, opts.dikarouteProviderId).split("/").pop()!] =
        entry;
  ...
  if (pendingStatic.length > 0) {
    console.warn(
      `[dikaroute-plugin] ${pendingStatic.length} combo(s) in the static catalog could not resolve all nested combo-refs after ${MAX_STATIC_COMBO_PASSES} passes; they will be omitted.`
    );
  }
    ...
    for (const autoCombo of rawAutoCombos) {
      ...
      if (models[key]) {
        ...
        if (!reportedCollisions.has(key)) {
          ...
          console.warn(
            `[dikaroute-plugin] auto combo key "${key}" collides with an existing model; auto combo wins.`
          );
        }
      }
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createDikaRouteConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/dikaroute-<providerId>.json`.
...
interface DikaRouteDiskSnapshot {
  v: 2;
  ...
  rawEnrichment: Array<[string, DikaRouteEnrichmentEntry]>;
  rawCompressionCombos: DikaRouteCompressionCombo[];
  rawConnections: DikaRouteProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `dikaroute-${providerId}.json`);
}
...
export type DikaRouteDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<DikaRouteFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type DikaRouteDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<Omit<DikaRouteFetchCacheEntry, "expiresAt"> | undefined>;
...
export const defaultDiskSnapshotWriter: DikaRouteDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: DikaRouteDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: DikaRouteDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<DikaRouteDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
...
export const noopDiskSnapshotWriter: DikaRouteDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `dikaroute-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `dikaroute-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[dikaroute-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[dikaroute-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[dikaroute-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export const noopDiskSnapshotReader: DikaRouteDiskSnapshotReader = async () => undefined;

export type DikaRouteReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: DikaRouteReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultDikaRouteModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultDikaRouteCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `DikaRouteFetchCache`). Pass the same Map the
 *                          provider hook owns to dedupe round-trips.
 *   - `logger`           — `{warn}` sink for breadcrumb capture in tests.
 *                          Defaults to `console`.
 */
export function createDikaRouteConfigHook(
  opts?: DikaRoutePluginOptions,
  deps: {
    readAuthJson?: DikaRouteReadAuthJson;
    fetcher?: DikaRouteModelsFetcher;
    ...
    compressionMetaFetcher?: DikaRouteCompressionMetaFetcher;
    providersFetcher?: DikaRouteProvidersFetcher;
    diskSnapshotReader?: DikaRouteDiskSnapshotReader;
    diskSnapshotWriter?: DikaRouteDiskSnapshotWriter;
    now?: () => number;
    cache?: DikaRouteFetchCache;
  ...
  return async (input: Config) => {
    ...
    if (existingProviders && existingProviders[resolved.providerId] !== undefined) {
      logger.warn(
        `[dikaroute-plugin] config shim skipped: provider.${resolved.providerId} already set by user`
      );
      return;
    }
    ...
    if (authJson === null) {
      logger.warn("[dikaroute-plugin] config shim: auth.json failed to parse; treating as missing");
      authJson = undefined;
    }

    // Try both prefixed (e.g. opencode-dikaroute) and unprefixed (e.g. dikaroute)
    // keys so a user who ran `/connect dikaroute` before the auto-prefix fix
    ...
    if (!apiKey) {
      ...
      logger.warn(
        `[dikaroute-plugin] config shim skipped: no apiKey for providerId=${resolved.providerId}`
      );
      return;
    }
    // Management-plane catalog reads may use a narrower read-only token.
    ...
    if (!baseURL) {
      logger.warn(
        `[dikaroute-plugin] config shim skipped: no baseURL for providerId=${resolved.providerId}`
      );
      return;
    }
    ...
    if (cached && cached.expiresAt > t) {
      ...
      try {
        ...
        logger.warn(
          "[dikaroute-plugin] config shim: /v1/models fetch failed; publishing stub provider entry",
          err
        );
        rawModels = [];
      ...
      try {
        ...
        logger.warn(
          "[dikaroute-plugin] config shim: /api/combos fetch failed; publishing models-only static catalog",
          err
        );
      }
      ...
      if (wantEnrichment) {
        try {
          ...
          logger.warn(
            "[dikaroute-plugin] config shim: /api/pricing/models fetch failed; publishing raw-id static catalog",
            err
          );
        }
      }
      ...
      if (wantCompressionMeta) {
        try {
          ...
          logger.warn(
            "[dikaroute-plugin] config shim: /api/context/combos fetch failed; publishing combos without compression suffix",
            err
          );
        }
      }
      ...
      if (wantUsableOnly) {
        try {
          ...
          logger.warn(
            "[dikaroute-plugin] config shim: /api/providers fetch failed; usableOnly filter disabled for this refresh",
            err
          );
        }
      }
      ...
      if (modelsFetchThrew && wantDiskCache) {
        ...
        if (snapshot && snapshot.rawModels.length > 0) {
          logger.warn(
            `[dikaroute-plugin] config shim: /v1/models unreachable; using stale disk cache (${snapshot.rawModels.length} models)`
          );
    ...
    if (features.mcpAutoEmit === true) {
      ...
      if (!mcpKey) {
        logger.warn(
          `[dikaroute-plugin] mcp auto-emit skipped: no Bearer token for providerId=${resolved.providerId}`
        );
      } else {
        ...
        if (inputWithMcp.mcp[resolved.providerId] !== undefined) {
          logger.warn(
            `[dikaroute-plugin] mcp auto-emit skipped: mcp.${resolved.providerId} already set by user`
          );
        } else {
