...
import {
  ...
  diskSnapshotPath,
  type DikaRouteFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<DikaRouteFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dikaroute-disk-perms-"));
