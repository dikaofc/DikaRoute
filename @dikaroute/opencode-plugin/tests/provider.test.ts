 ...
 *
 * Covers `createDikaRouteProviderHook(opts, deps)`:
...
const FIXTURE: DikaRouteRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: DikaRouteRawModelEntry[]): DikaRouteModelsFetcher & {
  callCount: () => number;
...
test("createDikaRouteProviderHook: default providerId is 'dikaroute'", () => {
  const hook = createDikaRouteProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-dikaroute");
});

test("createDikaRouteProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createDikaRouteProviderHook(
    { providerId: "dikaroute-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createDikaRouteProviderHook(
    { providerId: "dikaroute-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-dikaroute-preprod");
  assert.equal(b.id, "opencode-dikaroute-local");
});
...
test("models: extracts apiKey from ctx.auth (type=api) and calls fetcher with it", async () => {
  ...
  // ("dikaroute"), not the OC-gate-prefixed hook.id ("opencode-dikaroute") —
  // that prefix must never leak into anything DikaRoute's server parses.
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createDikaRouteProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  // dikarouteProviderId ("dikaroute") — the OC-gate prefix ("opencode-")
  // must stay OC-internal (hook.id / AuthHook.provider) and never leak into
  // anything DikaRoute's own server parses for credential lookup.
  const claude = out["dikaroute/claude-primary"];
  ...
  assert.equal(claude.id, "dikaroute/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "dikaroute");
  ...
  // cost is zeroed (DikaRoute /v1/models has no pricing)
