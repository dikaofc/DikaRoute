 ...
 *
 * Covers `createDikaRouteFetchInterceptor` (URL-prefix gating, header merge,
...
import { createDikaRouteAuthHook, createDikaRouteFetchInterceptor } from "../src/index.js";
...
test("createDikaRouteFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createDikaRouteFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createDikaRouteFetchInterceptor({
      apiKey: KEY,
...
test("createDikaRouteFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDikaRouteFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDikaRouteFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDikaRouteFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({
      apiKey: KEY,
...
test("createDikaRouteFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDikaRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createDikaRouteAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createDikaRouteAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createDikaRouteAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
