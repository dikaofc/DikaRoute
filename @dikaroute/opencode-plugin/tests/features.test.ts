 ...
 *
 * Surfaces tested:
 *   - `parseDikaRoutePluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createDikaRouteProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createDikaRouteConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultDikaRouteEnrichmentFetcher,
  defaultDikaRouteCompressionMetaFetcher,
  ...
  type DikaRouteCompressionCombo,
...
test("parseDikaRoutePluginOptions: empty features object → preserved", () => {
  const r = parseDikaRoutePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseDikaRoutePluginOptions: all boolean features set → preserved", () => {
  const r = parseDikaRoutePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseDikaRoutePluginOptions: mcpToken string → preserved", () => {
  const r = parseDikaRoutePluginOptions({
...
test("parseDikaRoutePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseDikaRoutePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @dikaroute\/opencode-plugin options/
  );
});

test("parseDikaRoutePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseDikaRoutePluginOptions({
...
test("parseDikaRoutePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseDikaRoutePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @dikaroute\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: DikaRouteRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed dikarouteProviderId.
  const m = out["dikaroute/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["dikaroute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: DikaRouteCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["dikaroute/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-dikaroute": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-dikaroute"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dikaroute"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dikaroute"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-dikaroute": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-dikaroute"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createDikaRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-dikaroute-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dikaroute-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultDikaRouteEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultDikaRouteEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultDikaRouteEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultDikaRouteEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultDikaRouteCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultDikaRouteCompressionMetaFetcher("", "sk", 100);
...
test("defaultDikaRouteEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultDikaRouteEnrichmentFetcher(
...
test("defaultDikaRouteEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultDikaRouteEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultDikaRouteEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): DikaRouteEnrichmentMap {
  const map: DikaRouteEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: DikaRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as DikaRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as DikaRouteRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: DikaRouteRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as DikaRouteRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: DikaRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as DikaRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as DikaRouteRawModelEntry,
    { id: "cx/gpt-5.5" } as DikaRouteRawModelEntry,
    { id: "codex/gpt-5.5" } as DikaRouteRawModelEntry,
    { id: "pol/openai-large" } as DikaRouteRawModelEntry,
    { id: "pollinations/openai-large" } as DikaRouteRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
