 ...
 *
 * Covers `createDikaRouteConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@dikaroute/opencode-provider`).
...
import {
  ...
  createDikaRouteConfigHook,
  createDikaRouteProviderHook,
  DikaRoutePlugin,
  resolveDikaRoutePluginOptions,
  type DikaRouteCombosFetcher,
  type DikaRouteEnrichmentEntry,
  type DikaRouteEnrichmentFetcher,
  type DikaRouteEnrichmentMap,
  type DikaRouteFetchCache,
  type DikaRouteModelsFetcher,
  type DikaRouteProviderConnection,
  type DikaRouteProvidersFetcher,
  type DikaRouteRawCombo,
  type DikaRouteRawModelEntry,
  type DikaRouteReadAuthJson,
  type DikaRouteStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: DikaRouteRawModelEntry = {
...
const MODEL_GEMINI: DikaRouteRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: DikaRouteRawCombo = {
...
): DikaRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): DikaRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: DikaRouteRawModelEntry[]
): DikaRouteModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DikaRouteModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: DikaRouteRawCombo[]
): DikaRouteCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DikaRouteCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): DikaRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): DikaRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: DikaRouteEnrichmentMap): DikaRouteEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DikaRouteEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): DikaRouteEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDikaRouteConfigHook(
  ...
  const provider = (input as { provider: Record<string, DikaRouteStaticProviderEntry> }).provider;
  const entry = provider["opencode-dikaroute"];
  assert.ok(entry, "input.provider['opencode-dikaroute'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "DikaRoute");
  ...
  const claude = entry.models["opencode-dikaroute/claude-sonnet-4-6"];
  ...
  const combo = entry.models["dikaroute/claude-tier"];
...
//     prefixed (`opencode-dikaroute`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `dikaroute` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-dikaroute`.
  const readAuthJson = stubReadAuthJson({
    dikaroute: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute" }, // resolves to opencode-dikaroute internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    dikaroute: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, DikaRouteStaticProviderEntry> }).provider[
    "opencode-dikaroute"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual DikaRoute",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-dikaroute": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-dikaroute"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-dikaroute/claude-sonnet-4-6",
    "opencode-dikaroute/gemini-3-flash",
  ]);
  assert.equal(entry.models["dikaroute/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-dikaroute-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createDikaRouteConfigHook(
    { providerId: "dikaroute-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createDikaRouteConfigHook(
    { providerId: "dikaroute-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, DikaRouteStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-dikaroute-prod"], "prod block present");
  assert.ok(provider["opencode-dikaroute-preprod"], "preprod block present");
  assert.equal(provider["opencode-dikaroute-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-dikaroute-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-dikaroute-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-dikaroute-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-dikaroute-prod"],
    provider["opencode-dikaroute-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createDikaRouteConfigHook(
    { providerId: "dikaroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createDikaRouteProviderHook(
    { providerId: "dikaroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: DikaRouteFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @dikaroute/opencode-provider", () => {
  const resolved = resolveDikaRoutePluginOptions({
    providerId: "dikaroute",
    displayName: "DikaRoute",
  });
  ...
  assert.equal(block.name, "DikaRoute");
  ...
  const claude = block.models["opencode-dikaroute/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["dikaroute/claude-tier"], undefined);
  assert.ok(block.models["opencode-dikaroute/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveDikaRoutePluginOptions({ providerId: "dikaroute" });
  const withDate: DikaRouteRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-dikaroute/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-dikaroute/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveDikaRoutePluginOptions({ providerId: "dikaroute" });
  const TEXT_ONLY: DikaRouteRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["dikaroute/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: DikaRoutePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("DikaRoutePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await DikaRoutePlugin({} as never, { providerId: "dikaroute" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dikaroute": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-dikaroute"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, DikaRouteEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-dikaroute/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-dikaroute/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["dikaroute/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-dikaroute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: DikaRouteProviderConnection[]): DikaRouteProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: DikaRouteProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): DikaRouteProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: DikaRouteProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: DikaRouteRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: DikaRouteRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-dikaroute/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-dikaroute/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot DikaRoute hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-dikaroute/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    { providerId: "dikaroute", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, DikaRouteStaticProviderEntry> }).provider[
    "opencode-dikaroute"
  ];
  assert.equal(
    entryA.models["opencode-dikaroute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, DikaRouteStaticProviderEntry> }).provider[
    "opencode-dikaroute"
  ];
  assert.equal(
    entryB.models["opencode-dikaroute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveDikaRoutePluginOptions({ providerId: "dikaroute" });
  const rawModels: DikaRouteRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: DikaRouteRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["dikaroute/parent"];
