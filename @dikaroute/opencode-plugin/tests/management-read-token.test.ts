...
const RAW_MODELS: DikaRouteRawModelEntry[] = [
  {
    id: "openai/gpt-test",
...
test("options: managementReadToken is accepted and preserved", () => {
  const parsed = parseDikaRoutePluginOptions({ managementReadToken: MANAGEMENT_READ_TOKEN });
...
test("provider hook: management GET fetchers use managementReadToken while /v1 uses apiKey", async () => {
  ...
  const enrichmentFetcher: DikaRouteEnrichmentFetcher = async (_baseURL, token) => {
    calls.push(["pricing", token]);
    return new Map();
  };
  const compressionMetaFetcher: DikaRouteCompressionMetaFetcher = async (_baseURL, token) => {
    calls.push(["context", token]);
    return [];
  };
  const providersFetcher: DikaRouteProvidersFetcher = async (_baseURL, token) => {
...
test("config hook: managementReadToken stays out of provider inference and MCP config", async () => {
  ...
  const hook = createDikaRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-dikaroute": { type: "api" as const, key: API_KEY },
      }),
      fetcher: async (_baseURL, token) => {
        calls.push(["models", token]);
        return RAW_MODELS;
  ...
  assert.equal(input.provider?.["opencode-dikaroute"]?.options?.apiKey, API_KEY);
  assert.equal(
    input.mcp?.["opencode-dikaroute"]?.headers?.Authorization,
    `Bearer ${API_KEY}`,
...
test("disk cache: snapshot written under management token A is rejected under token B", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dikaroute-token-snapshot-"));
  ...
  try {
    ...
    const tokenAHook = createDikaRouteConfigHook(
    ...
    const tokenBHook = createDikaRouteConfigHook(
    ...
    assert.deepEqual(
      input.provider?.["opencode-dikaroute"]?.models,
      {},
