...
function combo(models: DikaRouteRawCombo["models"]): DikaRouteRawCombo {
...
test("parseDikaRoutePluginOptions: visibleModels string[] → preserved", () => {
...
test("parseDikaRoutePluginOptions: hiddenModels string[] → preserved", () => {
  const r = parseDikaRoutePluginOptions({
...
test("parseDikaRoutePluginOptions: both lists together → preserved", () => {
  const r = parseDikaRoutePluginOptions({
    features: {
...
test("parseDikaRoutePluginOptions: empty string in visibleModels → rejects", () => {
  assert.throws(
    () =>
      parseDikaRoutePluginOptions({
...
test("parseDikaRoutePluginOptions: empty string in hiddenModels → rejects", () => {
  assert.throws(
    () =>
      parseDikaRoutePluginOptions({
...
test("parseDikaRoutePluginOptions: unknown features key still rejects (strict invariant)", () => {
  assert.throws(
    () =>
      parseDikaRoutePluginOptions({
...
const FAKE_RAW_MODELS: DikaRouteRawModelEntry[] = [
...
test("buildStaticProviderEntry: no allowlist → all models emitted", () => {
  const opts = resolveDikaRoutePluginOptions({ features: {} });
...
test("buildStaticProviderEntry: visibleModels filters to only listed IDs", () => {
  const opts = resolveDikaRoutePluginOptions({
