 ...
 *
 * Covers the `createDikaRouteAuthHook(opts)` factory and its loader behaviour
...
import { createDikaRouteAuthHook } from "../src/index.js";

test("createDikaRouteAuthHook: default providerId is 'dikaroute'", () => {
  const hook = createDikaRouteAuthHook();
  assert.equal(hook.provider, "opencode-dikaroute");
});

test("createDikaRouteAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createDikaRouteAuthHook({ providerId: "dikaroute-preprod" });
  assert.equal(hook.provider, "opencode-dikaroute-preprod");
});

test("createDikaRouteAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createDikaRouteAuthHook();
  ...
  assert.equal(m.label, "DikaRoute API Key");

  const custom = createDikaRouteAuthHook({ providerId: "dikaroute-preprod" });
  assert.equal(custom.methods[0].label, "DikaRoute (opencode-dikaroute-preprod) API Key");
});

test("createDikaRouteAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("dikaroute"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createDikaRouteAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
