 ...
 *
 * Validates that two `DikaRoutePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@dikaroute/opencode-plugin", {"providerId": "dikaroute-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@dikaroute/opencode-plugin", {"providerId": "dikaroute-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { DikaRoutePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof DikaRoutePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await DikaRoutePlugin(fakeInput, {
    providerId: "dikaroute-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await DikaRoutePlugin(fakeInput, {
    providerId: "dikaroute-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-dikaroute-prod");
  assert.equal(b.auth?.provider, "opencode-dikaroute-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await DikaRoutePlugin(fakeInput, { ...opts });
  const second = await DikaRoutePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => DikaRoutePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await DikaRoutePlugin(fakeInput, {
